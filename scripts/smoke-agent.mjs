...
async function main() {
  ...
  if (!agentId) {
    console.error("找不到 agent，先在 Agent 管理页创建一个");
    process.exit(1);
  }
  ...
  check(
    "task claim",
    (await api("POST", `${B}/tasks/claim`, { channel: CH, task_numbers: [taskNum] })).data?.results?.[0]?.status ===
      "claimed",
  );
  check(
    "task update-status",
    (await api("POST", `${B}/tasks/update-status`, { channel: CH, number: taskNum, status: "in_review" })).ok,
  );
  ...
  if (taskNum != null)
    await api("POST", `${B}/tasks/update-status`, { channel: CH, number: taskNum, status: "closed" });
