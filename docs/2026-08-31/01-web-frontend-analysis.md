...
| 14 | **密码策略不一致** ✅ 2026-09-05 已修复（三页面收敛单点）：ForgotPasswordPage.vue:41 要求 ≥6 位 vs RegisterPage.vue:44-51 ≥8 位+字母数字。⚠️ 2026-09-01 server P1.20 补齐 forgot/reset-password 端点（reset 走 server `validatePassword` ≥8+字母+数字）后，从「文案不一致」升级为实锤的「客户端过、server 拒」——`abc123`（6–7 位）、`abcdefgh`（纯字母）均过客户端被 server 400；失败文案可透传（:51）故非静默断裂，修法仍是对齐 ≥8+字母+数字（§8.2） | — |
| 15 | **危险操作确认不一致** ✅ 2026-09-05 已修复（三处统一 ConfirmDialog）：WorkspaceMembers.vue:89-97 移除成员无确认（同站 ChannelManagement 有 ConfirmDialog）；SecuritySettings.vue:81 用原生 `confirm()`；IntegrationSettings 撤销令牌无确认 | — |
| 16 | **ThreadView 无加载态** ✅ 2026-09-05 已修复：加载中呈空白 | ThreadView.vue:54-66 |
...
> **P1-14 修复记录（2026-09-05，密码策略不一致）**：新建 `lib/passwordPolicy.ts` 导出 `validatePasswordPolicy(pw): string | null`——与 server `validators.ts` validatePassword **同源同序同文案**（≥8 位→「密码至少 8 位」/ 含字母 / 含数字），注释锚定 server 同源口径（server 仍是唯一强制点）。**三页面收敛单点**：①ForgotPasswordPage `handleReset` ≥6 预检替换为 helper（实锤样例 `abc123`/`abcdefgh` 双双本地拦截，placeholder 同步「至少8位，含字母和数字」）；②**ProfileSettings 改密为审计未点名的同病第三处**——原仅验 ≥8 缺字母数字（`abcdefgh` 过客户端被 server 400「密码需包含数字」），同 helper 接线 + placeholder 同步；③RegisterPage 三段内联（:44-51）迁 helper，规则逐条等价、合并文案「字母和数字」拆为 server 同款单条文案。全仓 grep 复扫客户端密码规则点共 3 处无第四处（PasswordStrength 为可视化提示不校验）。验证：vitest 71→**73**（新增 `lib/passwordPolicy.test.ts` +2：审计实锤样例四连拦截 + 合法放行边界）、vue-tsc 0 错误、biome 0 错误（3 文件 import 排序由 --write 归一，语义中性）、`vite build` 成功（import 重排后复跑终验）。取舍立此存照：客户端预检不挡「两次密码不一致」（RegisterPage 既有逻辑保留在 helper 之外——confirm 匹配是纯 UI 语义非 server 策略）；helper 不做强度评分（PasswordStrength 已有可视化，职责分离）。
>
> **GateGuard/工作目录实锤留痕**：验证命令一度在仓库根执行（commit 的 cd 残留）触发 npx 缓存版 vue-tsc ERR_PACKAGE_PATH_NOT_EXPORTED 假红——W-A2 留痕「验证必须锁定 packages/web」再次应验，cd 回后全绿。

> **P1-15 修复记录（2026-09-05，危险操作确认不一致）**：三处统一走全站 ConfirmDialog 样板（pending ref → `@confirm` 执行 + 清 ref → `@cancel` 清 ref，与 ChannelManagement 同构；ConfirmDialog 内嵌 Modal 自动获得 P1-5 的 focus trap/滚动锁定/焦点归还）。①**WorkspaceMembers 移除成员**——`removeTarget` ref，✕ 按钮改只设目标，确认后走既有 `removeMember`（owner 自保护与错误 msg 逻辑原样保留），文案「移除后该成员将失去此工作区的访问权限」；②**SecuritySettings 退出所有设备**——原生 `confirm()` 删除，`logoutAll` 成裸动作，按钮改开 `showLogoutAll` 弹窗，文案「将退出全部会话（包括当前设备）」；③**IntegrationSettings 撤销机器令牌**——`revokeTarget` ref，✕ 撤销按钮（W-A5 修复记录预告归此处）改确认流，文案「撤销后令牌立即失效，使用该令牌的 daemon 将无法连接」。验证：vue-tsc 0 错误、biome 0 错误（WorkspaceMembers import 排序 --write 归一后终验复跑全绿）、vitest 73/73、`vite build` 成功（页面级交互无测试设施，W-A7 口径以 build 为终验）。> **P1-16 修复记录（2026-09-05，ThreadView 无加载态）**：`loadThread` 加 `loading` 标志（finally 复位）——首载（parent 为 null）期间内容区显示 `MessageSkeleton`（头像+两行文本 ×5，与线程页形态天然契合，animate-shimmer 由 P0-3 补齐后生效），不再整块空白；父卡分支 `v-if` 改 `v-else-if` 串接。**取舍立此存照**：线程切换不做 parent 置空——旧线程内容在加载期间保持显示（stale-while-revalidate 防闪空，骨架仅在首载 parent 为 null 时出现）；`loadThread` 开头顺带清 `error` 残留——否则上一线程失败后切新线程，加载期间（error 非空 + parent 为 null）会误显上一线程的整页错误态。验证：vue-tsc 0 错误、biome 0 错误、vitest 73/73、`vite build` 成功（一次过零返工；模板骨架分支无测试面，W-A7 口径以 build 为终验）。**至此 §4.3 状态完整性/UX 一致性 #11~#16 全部清零。**
...
| 18 | **测试盲区** ✅ 2026-09-05 已修复（41→130 用例，+57）：authStore / api(CSRF) / agent·channel·computer·notification·task·terminal 6 个 store / 3 个 composables 全部零测试；消息渲染安全链（DOMPurify+md）无回归网 | 现有 41 用例集中在 ws 链路 |
| 19 | **消息明文缓存 localStorage** ✅ 2026-09-05 已修复（口径定稿 + 登出清盘双防线）：频道消息与离线队列明文落盘（切频道/刷新不丢的设计权衡） | messageStore.ts:26-35,216；知情风险，建议确认口径或加上限/开关 |
...
> **P1-19 修复记录（2026-09-05，localStorage 消息缓存口径）**：审计给出的三个选项全数处置——**确认口径 + 上限核实 + 登出清盘（替代开关）**。①**口径定稿（记录于 `lib/message-cache.ts` 模块头）**：明文缓存 =「切频道/刷新不丢消息」的 UX 权衡，缓存的是本浏览器已对该账号展示过的消息，信任域 = 浏览器 profile 本地存储（与 httpOnly cookie 会话同域同浏览器，不新增暴露面）；上限已内置（CACHE_LIMIT 50/频道 + 离线队列仅落 queued/failed），「开关」不设——权衡收益覆盖全部用户、清盘已封住实质危害路径，开关属过度设计。②**跨账号残留不属权衡内（实锤）**：全部登出路径（UserMenu:46 / SecuritySettings:86 logoutAll / :121 改密）均 `authStore.logout(); router.push("/login")`，SPA 内跳转无整页刷新——换账号登录会读到旧账号消息缓存，旧账号离线草稿会以新账号身份被 flush 发出（跨账号泄漏，比隐私更实质）；且 SPA 登出后 store 单例跨登录存活，仅清 localStorage 不够——in-flight flush 失败会经 `persistPending` 把旧账号草稿**重写回** localStorage（清了等于没清）。③**落地（单点 lib，遵守「无 store 互 import」纪律）**：新建 `lib/message-cache.ts`——key 常量（`msgs_` 前缀/`pending_msgs_v1`）从 messageStore 迁入单点 + `clearMessageCaches()`（遍历清 `msgs_*` + pending key，UI 偏好键不误伤）+ `onMessageCachesCleared()` 回调注册点；messageStore 常量改由 lib 导入并在 store 创建时注册内存清盘回调（messages/lastSeenSeq/loadError/pendingByTarget 全复位）；authStore.logout() 调 `clearMessageCaches()`。验证：vitest 130→**138**（+8：lib 5——前缀清盘/UI 键保留/无 localStorage 安全/存储抛异常回调仍执行/回调抛错不阻断；messageStore 联动 2——clearMessageCaches 复位内存态+localStorage、幂等；authStore 1——logout 清缓存且 theme 不误伤）、vue-tsc 0 错误、biome 0 错误、`vite build` 成功（index 124.69 kB）。取舍立此存照：UI 偏好键（theme/sidebar/notif_prefs/terminal_panel/task view/onboarding）不属消息缓存口径，登出不误伤；notif_prefs 跨账号残留属偏好级非消息数据，不入本项；登出时 in-flight fetchHistory 若在清盘后 resolve 仍会写回一条缓存（罕见边界，下次登录 fetchHistory 归并以 server 为权威覆盖，接受并留痕）；登出清离线队列意味着未发送草稿随登出丢弃（正确取舍：草稿属账号会话，跨账号存活更糟）；401 拦截登出（§8.2 #2 遗留建议）落地时应复用同一 clearMessageCaches 入口。实锤留痕：messageStore 测试 harness 的 localStorage stub `key: () => null` 恒空（写于本项之前，仅支撑 get/set/remove 消费）——clearMessageCaches 遍历依赖 key(i) 抓出该行为差，harness 已对齐真实 Storage 语义。

> **P1-18 修复记录（2026-09-05，测试盲区）**：盲区盘点先复判——审计成文后 notificationStore（W-A1 +4）与 channelStore（P1-9 +3）已补测，taskStore 经 §8.2 #3 重判死代码「删除而非补测试」，故实际新覆盖面 = **authStore / api(CSRF) / agent / computer / terminal 5 面 + 3 个 composables + markdown 安全链**。①**markdown 安全链抽出 `lib/markdown.ts`**（renderMarkdown 单函数；github-dark.css 随链迁入），MarkdownContent.vue 改薄壳——使 XSS 回归网可在 node 测试直测：7 用例覆盖审计 §2.2 四环（html:false 转义 script/img 载荷、javascript: 协议链接拒绝、显式+linkify 链接统一 target=_blank/rel=noopener、已知语言 hljs 着色、未知语言 escapeHtml 降级不丢内容、「输出恒经 DOMPurify.sanitize」接线断言——dompurify mock 为记录式透传，断链即红）。②**api/index.test.ts 13 用例**：readCsrf 三态、CSRF double-submit 注入四态（POST 注入/GET 不带/无 cookie 不带/自定义 headers 合并）、ApiError 映射四态（status+文案透传/非 JSON 回退 statusText/Error 子类/2xx 解析）、apiGet qs 编码、uploadAttachment 三态（FormData+CSRF+错误）——真实模块 + vi.stubGlobal stub fetch/document，零 vi.mock。③**authStore 9 用例**：login 成败、logout（清镜像+CSRF 头有无双态）、updateUser 浅合并、持久化镜像恢复与损坏容错×2（`vi.resetModules()`+动态 import——模块级 savedUser 仅 import 时求值，静态导入恒为未登录）。④**agentStore 8 用例**：updateStatus 默认骨架（name=id 前 8 位/duty on/computerOnline true/presence 合成）、duty/computerOnline 跨更新保留、applyPresence upsert（id 缺省回退）、setAgents 替换语义、progress 双索引 # 剥离、clearProgress 三态（异名不误清/同名清双索引/无 agentName 只清频道槽）。⑤**computerStore 7 用例**：refresh 三级降级链（me → daemon/status computer null 归一 → connected:false 兜底不抛错）、ensure、claudeInstalled 真值表、runtimeCatalog。⑥**terminalStore 5 用例**：帧/历史 per-agent 隔离、appendObsFrame 超 500 截断保尾（505→500 首帧 seq=6）、setObsHistory slice(-500)（600→500 首帧 seq=101）、双 agent 隔离。⑦**composables**：usePolling 抽 `startPolling`（生命周期无关计时核心；usePolling 契约不变，消费方 ComputerView/OnboardingChecklist 零改动）+2 用例（周期执行/幂等 stop）；useMediaQuery node 回退 + LG_QUERY 契约 2 用例；useMentionSuggest 8 用例钉住 React parity 语义（@ 检测光标回溯、空格截断、新 @ 会话重拉、键盘边界不越界+preventDefault、Escape 关闭、私有频道 members 映射与 channelId 未加载守卫、scope 变化重载）。**数字**：vitest 73 → **130**（18 文件，+57），vue-tsc 0 错误、biome 0 错误、`vite build` 成功（index 124.32 kB，#17 拆包战果无回退）。取舍立此存照：node 环境无 DOM/组件挂载（vitest.config 明示「不引入 jsdom/happy-dom 保持依赖最小」），DOM 绑定面不入回归网——insertMention 原生 setter hack、document mousedown 处理、matchMedia change 订阅、usePolling 的生命周期接线（计时核心已由 startPolling 覆盖）；DOMPurify 库自身行为不经测（node 无 DOM 时 isSupported=false 恒等返回，测库无价值），以接线断言替代；taskStore 不补测试（§8.2 #3 死代码判删除）。实锤留痕：①初版「空格截断」用例误设文本 `hi @al`——@ 在当前词内回溯**会**命中（弹窗应开，代码正确测试错），修正为 `@al hi`（@ 在前一词被空格隔断才关闭）；②logout 用例最初断言 updateUser 后 `isAuthenticated` true——实际 updateUser 不置登录态（登录态仅由 login/镜像恢复驱动，与审计 §2.3/§5「仅凭 localStorage 判登录」同源事实），断言按真实语义修正；③AgentPresence 五值为 "idle/starting/working/off_duty/computer_offline"（无 "offline"）。

---

## 5. P2 清单（精选）

- **死代码** ✅ 2026-09-05 已删除（见下行修复记录；`wsDispatch.ts:1` 一项经复核已失效无需改动）：`panes/ActivityPane.vue`、`panes/SearchPane.vue`、`chat/SearchBar.vue` 全仓库零引用（侧栏两列化设计修订残留）；2026-09-04 增补：`message/MentionPopover.vue` 全仓零引用（P1-3 排查深色孤岛时发现，同款深色弹层系 chat/MentionPopup 的前身残留）；`messageStore.ts:113-130` `sendMessage` 死代码（无 clientNonce，与 enqueuePending 双轨）；`wsDispatch.ts:1` 未使用 import（biome 可自动修）。2026-09-04 增补：`taskStore.ts` 整 store 与 `channelStore.joinChannel/leaveChannel`（传频道名 vs server 只认 UUID）亦为死代码，与 server 报告 §3 drift #7/#8 互证，建议一并删除（§8.2）。

> **P2 死代码批修复记录（2026-09-05）**：删除前逐项 grep 复核（审计成文后历经 P0-2/P1-9/P1-18 等多批改动，断言须对当前树重验）。①**四个孤儿组件**（panes/ActivityPane、panes/SearchPane、chat/SearchBar、message/MentionPopover）全 src 零引用属实，其 import 均为共享模块（api/formatTime/MarkdownContent/uiStore/notificationStore 等），删除无二级孤儿；②**taskStore.ts** 整 store 仅余 `stores/index.ts:7` re-export（零组件消费、零测试引用），摘除 re-export 后删文件；③**channelStore joinChannel/leaveChannel**（:73-85）零调用方属实，连函数体带导出删除；④**messageStore sendMessage** 零调用方属实——审计行号 :113-130 已漂移至 :157-174（P1 批改动所致），`MemberProfileBody.vue:207` 的同名函数为组件内无关本地函数（按钮消费），非引用，删除；⑤**wsDispatch.ts:1 未使用 import 已失效**——P1-9 重写未读分支时顺带收敛，现行 4 个 import（threadBufferKey/useAgentStore/useChannelStore/useMessageStore）全在用，无需改动。**删除后二级孤儿排查**：`joinedChannels` 失去全部读方（仅 fetchChannels 写入、store 导出无消费）成为只写状态——不在本清单点名范围，按「点名才动」纪律保留，立此存照为后续清理候选；`TaskStatus` 在 web 零残留（shared 包由 server 侧继续使用，不动）。验证：vue-tsc 0 错误、biome 0 错误（6 info 均既有基线，#18 批新增 api 测试占 1 条）、vitest 138/138（19 文件，删除项零测试面牵连）、`vite build` 成功无警告（index 124.22 kB，#17 拆包战果无回退）。§5 余量：emoji 图标、useMentionSuggest 假候选/防抖、OnboardingChecklist 恒 false、placeholder-as-label、日期分隔线/DM 虚拟化、ListPage 公共件、wsManager CONNECTING/send、unhandled rejection/errorHandler、401 拦截（§8.2 #2 升 P1 建议）、任意字号/字重、dev proxy 提示——均未动。
- **emoji 当图标约 120 处 / 50 文件** ✅ 2026-09-05 已迁移 lucide（见下行修复记录）：（Toast ✅⚠️❌、EmptyState 💬、密码可见性 🙈/👁、状态灯 🟢⚪），跨平台渲染不一致；同时存在 27 处手写 inline SVG（无图标库依赖）。建议统一引入图标库（项目已有 SVG 基础，迁移成本低）。

> **P2 图标库迁移修复记录（2026-09-05，emoji 当图标 + 内联 SVG → @lucide/vue）**：**选型实锤**——用户指定 lucide；`lucide-vue-next` 已被官方弃用（1.0.0 为弃用占位，npm 弃用声明「Please use @lucide/vue instead」），改装 **`@lucide/vue@1.40.0`**（官方 Vue3 组件库、ESM 具名导出、tree-shakeable，peer vue ≥3.0.1）。**三分法账本**（实测 emoji 94 处/38 文件，审计 120/50 偏高系含注释/测试口径）：①**UI 图标位 ~60 处迁 lucide**——Toast kindStyles.icon 改存组件（Info/CircleCheck/TriangleAlert/CircleX）；EmptyState icon prop 改语义名（alert/message/search/users/mail/zap/clock，内部映射单点控制，11 消费方同批改值）；通知 TYPE_ICON 同构映射抽 `lib/type-icons.ts` 共享（NotificationBell+ActivityView，markRaw）；4 个数据驱动 tab 文件（SidebarRail/MobileTabBar/SettingsLayout/AdminPanel）`:d` 路径串改 lucide 组件引用；13 文件 27 处手写 Heroicons 风格内联 SVG 全部替换（Menu/Search/Lock/Sun/Moon/LogOut/ChevronDown/ChevronLeft/Plus/SquareTerminal/ClipboardList/Users/Settings/Paperclip/Send 等）；关闭按钮 ✕×12→X；密码眼睛 🙈/👁→EyeOff/Eye；经理徽标 👔×5→Crown；📄→FileText、💻→Monitor、🔍→Search、⏳→LoaderCircle/Hourglass、💭→Sparkles、😀→Smile、🗑→Trash2、已复制✓→Check、✓/○→Check/Circle；②**内容协议豁免（web 单端改会破功能）**：D4 ⏳ 进度消息（wsDispatch `startsWith("⏳")` 硬删判断）、T8 派单卡片（MessageRow startsWith 📋/✅/🚫）、EMOJI_CHOICES 表情反应数据、AgentTerminalPanel `<option>` 内 🟢⚪（原生 select 不能含组件元素）、历史落库消息内容；③**toast 文案剥离**：wsDispatch 3 处 ⏳⚠️❌ 前缀剥掉（全站其余 ~57 处 toast 均无 emoji 前缀，向多数派收敛，图标语义由 Toast 图标位承担；wsDispatch.test 无文案断言，测试安全）。**残留复扫**：emoji 94→17 行，全部为豁免类（内容协议 4 + 注释 6 + 原生 select 1 + 测试数据 6）。**数字**：48 文件 +235/-315；build 成功无警告，index 124.69→**131.35 kB**（gzip 38.79→40.38，+1.6 kB gzip 为入口 chunk 常驻图标——AppLayout/SidebarRail/Toast/EmptyState 居民，远低于预估），vendor/懒块无回退。验证：vue-tsc 0 错误、biome 0 错误（--write 归一 11 文件 import 排序后复跑 6 info 既有基线）、vitest 138/138、`vite build` 成功。取舍立此存照：lucide 组件尺寸沿用 Tailwind class（w-4 h-4 等）+ currentColor 天然双模式；EmptyState icon 保持 string 契约而非组件对象（消费方传字符串，避免 11 处 markRaw 样板）；🟢⚪ 状态灯若后续脱离原生 select 可改 CSS dot。
...
**第三梯队（状态与测试，持续）**：~~messageStore 加 error 字段消灭「错误伪装成空态」~~（✅ 2026-09-05 落地，见 §4.3 P1-11 修复记录——messageStore.loadError + ChannelView/DmView/5 页面共 7 个消费面全量接线）；~~highlight.js 按需注册拆包~~（✅ 2026-09-05 落地，见 §4.4 P1-17 修复记录——index 实测 1,238→124 kB，远优于预估的 400 kB）；~~authStore + api + 6 个 store 补单测~~（✅ 2026-09-05 落地，见 §4.4 P1-18 修复记录——41→130 用例，XSS 安全链回归网建成）；~~确认 localStorage 消息缓存口径~~（✅ 2026-09-05 落地，见 §4.4 P1-19 修复记录——口径定稿 + 登出清盘双防线，跨账号残留封堵）。**第三梯队至此全部完成，§4 全表（P0 四项 + P1 十九项）闭环。**
...
- 2026-09-05 P1-18（§4.4 #18 测试盲区）落地复测：73 → **130 用例**（18 文件，+57：markdown 安全链 7 + api/CSRF 13 + authStore 9 + agentStore 8 + computerStore 7 + terminalStore 5 + usePolling 2 + useMediaQuery 2 + useMentionSuggest 8 + taskStore 按 §8.2 判死代码不补），vue-tsc 0 错误、biome 0 错误、`vite build` 成功（index 124.32 kB 无回退）。安全链抽 `lib/markdown.ts` + usePolling 抽 `startPolling` 两处可测性重构（契约均不变）。§4 余量仅 §4.4 #19（localStorage 消息缓存口径，知情风险确认项）。
- 2026-09-05 P1-19（§4.4 #19 localStorage 消息缓存口径）落地复测：130 → **138 用例**（+8：lib/message-cache 5 + messageStore 登出联动 2 + authStore logout 清盘 1），vue-tsc 0 错误、biome 0 错误、`vite build` 成功（index 124.69 kB）。口径定稿（明文缓存=UX 权衡、信任域=浏览器 profile、上限已内置）+ 新建 `lib/message-cache.ts` 单点 + 登出清盘双防线（authStore.logout 清 storage + 回调联动清 messageStore 内存态，封堵「换账号读旧账号缓存/旧账号草稿以新账号身份发出」跨账号泄漏）。**至此 §4 全表闭环：P0 四项 + §4.1 八项 + §4.2 两项 + §4.3 六项 + §4.4 三项 + §8.3 适配七项全部清零**；余量仅 §8.2 #2（401 拦截登出，P1 升级建议）与 §5 P2 卫生批。
