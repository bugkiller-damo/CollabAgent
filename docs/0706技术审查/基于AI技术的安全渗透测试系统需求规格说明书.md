...
| **比赛目标** | 完成大赛全链路演示 | 按 7 月初赛（多目标渗透+链式操作）、8 月复赛（平台+引擎联调+病例库）、9 月决赛（全链路优化+路演）的节奏，逐步交付可演示版本 |
...
| **L3 — 人工决策** | CVSS ≥ 9.0 Exploit、跨子公司操作、全局策略变更 | 必须等待人工审批；超时按"超时默认行为"处理；子公司智能体可预设默认动作 |
| **L4 — 禁止授权** | 禁止操作清单中的行为（破坏性测试、DoS、数据篡改等） | 硬编码禁止，智能体无权决策，人工也无法临时授权 |

> **预设策略同步**：子公司管理员在在线时可将 L3 默认行为预配置至集团中枢同步；离线期间智能体按最近一次同步的预设策略执行。预设策略变更记录审计日志。
...
| **人工介入** | 人 ↔ 协作平台 | Web UI | 通信层 | 登录参与决策、审批、查看报告 |
...
{
  "versions": [
    {"version": "v1.0", "status": "active", "deprecated_at": null, "sunset_at": null},
    {"version": "v1.1", "status": "active", "deprecated_at": null, "sunset_at": null}
  ],
  "latest": "v1.1"
}
```
...
| 攻击图查询 | `GET` | `/api/v1/tasks/{task_id}/attack-graph` | 实时获取渗透攻击图（支持增量获取） |
| 日志获取 | `GET` | `/api/v1/tasks/{task_id}/logs` | 获取任务完整智能体运行日志 |
| 发送消息 | `POST` | `/api/v1/tasks/{task_id}/msg` | 给智能体发送消息 |
| 文件上传 | `POST` | `/api/v1/files` | 上传附件（PoC、截图、报告等） |
...
| 批量操作 | `POST` | `/api/v1/tasks/batch` | 批量创建/控制多个渗透任务 |
...
```json
{
  ...
  "progress": {
    "phase": "exploitation",
    "percent": 65, 
    "active_agents": ["recon", "execution", "privilege_escalation"], 
...
**action 可选值**：`pause`（暂停，保留所有现场状态）、`resume`（恢复执行）、`stop`（优雅停止，保存检查点和中间结果，支持后续恢复）、`terminate`（强制终止，不保存检查点，立即释放所有资源）

> `stop` 与 `terminate` 区别：`stop` 在终止前保存完整检查点（含已发现漏洞、攻击图快照、智能体状态），后续可通过 `resume` 恢复执行；`terminate` 立即终止所有活动连接与进程，不保存检查点。两者对应的状态枚举均为 `terminated`。
...
| `status` | string | 否 | 按状态过滤，多值逗号分隔。可选：`pending`（排队中）、`running`（执行中）、`paused`（已暂停）、`completed`（已完成）、`failed`（失败）、`terminated`（已终止） |
...
```
GET /api/v1/tasks/{task_id}/results?page=1&page_size=50
Authorization: Bearer {token}
```
...
|------|------|:----:|------|
| `page` | int | 否 | 页码，默认 `1` |
| `page_size` | int | 否 | 每页漏洞数，默认 `50`，最大 `200` |

**说明**：任务状态为 `completed` 时返回完整结果；任务仍在执行中（`running`/`pending`/`paused`）仅返回已确认的漏洞摘要，不包含完整攻击路径与 PoC。`vulnerabilities` 数组支持分页，`pagination` 对象指示总页数。
...
| `agent` | string | 否 | 按智能体角色过滤，多值逗号分隔。可选：`recon`、`planner`、`execution`、`privilege_escalation`、`lateral_move`、`coordinator` |
...
**说明**：向任务中的指定智能体发送人工指令或消息。消息将被传递到目标智能体的推理上下文，智能体在下一轮推理时处理。
...

{
  ...
  "content": "请暂停对 192.168.1.100 的扫描，优先处理 web-server-02",
  "priority": "HIGH"
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `target_agent` | string | 否 | 目标智能体角色。省略时发送给协调智能体分发。可选：`recon`/`planner`/`execution`/`privilege_escalation`/`lateral_move` |
| `content` | string | 是 | 消息正文，建议简明扼要描述指令 |
| `priority` | string | 否 | 消息优先级，默认 `MEDIUM`。可选：`LOW`/`MEDIUM`/`HIGH`/`CRITICAL` |
...
| 安全要求 | Token 仅存储于内存（不落盘）；传输强制 TLS 1.3；Token 不包含于日志 |
...
```
GET /api/v1/tasks/{task_id}/attack-graph?since=2026-07-07T10:00:00Z
Authorization: Bearer {token}
```

| 参数 | 类型 | 必需 | 说明 |
|------|------|:----:|------|
| `since` | ISO 8601 | 否 | 增量获取：返回指定时间点后的变更。省略时返回完整攻击图 |
...
```json
{
  ...
  "incremental": true,
  "nodes": [
    {"id": "web-server-01", "type": "entry", "status": "breached", "ip": "192.168.1.100", "breached_at": "2026-07-07T10:05:30Z"},
    {"id": "db-server-01",  "type": "internal", "status": "breached", "ip": "192.168.1.50", "breached_at": "2026-07-07T10:15:00Z"}
  ],
  "edges": [
    {"from": "web-server-01", "to": "db-server-01", "method": "SSH 密钥复用", "status": "success", "discovered_at": "2026-07-07T10:15:00Z"}
  ],
...
|------|------|:----:|------|
| `file` | binary | 是 | 文件内容 |
| `scope` | string | 是 | 使用场景：`case_evidence`（病例证据）/`screenshot`（截图）/`report`（报告）/`poc`（PoC 文件） |
...
| 限制项 | 阈值 |
|--------|:----:|
| 单文件最大 | 10 MB（超限存外部对象存储，仅存储引用链接） |
| 允许类型 | `application/pdf`、`image/png`、`image/jpeg`、`text/plain`、`application/zip` |
| 存储路径 | 按 `{scope}/{subsidiary_id}/{timestamp}_{file_id}` 组织 |

**超限处理**：超过 10MB 的文件，API 仍返回 `200 OK`，但 `storage_url` 指向外部对象存储路径，`oversized: true` 标记指示文件超出内联存储限制。

**响应**：

```json
{
  "file_id": "file-c4d5e6f7",
  "name": "screenshot-01.png",
  "size": 245760,
  "mime_type": "image/png",
  "storage_url": "hdfs://cases-bucket/evidence/subsidiary-a/20260707_file-c4d5e6f7.png",
  "oversized": false,
...
| `size` | int | 文件大小（字节） |
| `storage_url` | string | 文件存储路径。超限时指向外部对象存储 |
| `oversized` | bool | 是否超过 10MB 内联存储限制。`true` 时文件通过外部对象存储引用访问 |

#### 4.1.15 批量操作接口

统一入口，支持批量任务创建与控制。单次批量操作最多包含 **50 个**操作项（与 §3.2.6 批量任务上限对齐）。
  ...
  "operations": [
    {
      ...
      "task": {
        ...
        "targets": [{"target": "192.168.1.100", "asset_level": "core"}],
        "scan_profile": "standard",
        "priority": "HIGH"
      }
    },
    {
      ...
      "task": {
        "task_type": "vulnerability_scan",
        "targets": [{"target": "10.0.0.50", "asset_level": "important"}],
        "scan_profile": "quick",
        "priority": "MEDIUM"
      }
    }
...
{
  "results": [
    {"index": 0, "status": "accepted", "task_id": "task-a1b2c3d4"},
    {"index": 1, "status": "accepted", "task_id": "task-e5f6g7h8"}
  ],
  "failed_count": 0
}
```
...
{
  "operations": [
    {"action": "control", "task_id": "task-a1b2c3d4", "control": "pause", "reason": "目标网络抖动"},
    {"action": "control", "task_id": "task-e5f6g7h8", "control": "terminate", "reason": "超时未完成"}
  ]
}
```

**错误处理**：批量操作中单个操作失败不影响其余操作。响应中通过 `index` 字段对应原请求顺序，`status` 为 `failed` 时附加 `error` 字段说明失败原因。
...
**功能描述**：
提供告警规则配置、通知推送、告警历史查询与确认的 RESTful API。系统根据预定义的告警规则检测异常事件，通过配置的通知渠道向相关人员推送告警消息。
...
| 400 | `INVALID_PARAMETERS` | 请求参数不合法 |
| 401 | `AUTHENTICATION_FAILED` | Token 无效或过期 |
| 404 | `ALERT_NOT_FOUND` | 告警规则或记录 ID 不存在 |
| 409 | `ALERT_STATE_CONFLICT` | 告警状态不允许该操作（如对已确认告警再次确认） |
| 429 | `RATE_LIMIT_EXCEEDED` | 请求频率超过限流阈值 |
...
| 接口 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 创建告警规则 | `POST` | `/api/v1/alerts/rules` | 创建告警规则（触发条件、通知渠道、频率） |
| 查询告警规则 | `GET` | `/api/v1/alerts/rules` | 查询当前告警规则列表 |
| 更新告警规则 | `PUT` | `/api/v1/alerts/rules/{rule_id}` | 更新告警规则配置 |
| 删除告警规则 | `DELETE` | `/api/v1/alerts/rules/{rule_id}` | 删除告警规则 |
| 告警历史查询 | `GET` | `/api/v1/alerts/history` | 查询历史告警记录，支持多维过滤与分页 |
| 告警确认 | `POST` | `/api/v1/alerts/{alert_id}/acknowledge` | 确认告警（标记为已处理） |
| 告警统计 | `GET` | `/api/v1/alerts/stats` | 获取告警统计数据（按级别、时间聚合） |
...
{
  "name": "node_offline_detection",
  "description": "子公司智能体节点离线检测",
  "severity": "HIGH",
  "metric": "agent_heartbeat_missed",
  "condition": {
    "operator": ">=",
    "threshold": 2,
    "window_seconds": 60
...
| `condition.operator` | string | 是 | `>=`/`<=`/`==`/`>`/`<` |
| `condition.threshold` | float | 是 | 触发阈值 |
| `condition.window_seconds` | int | 否 | 评估时间窗口，默认 60s |
| `channels` | array | 是 | 通知渠道列表：`email`/`im`/`webhook`/`in_app` |
| `notify_interval_minutes` | int | 否 | 重复通知间隔，默认 30 |
...
```
GET /api/v1/alerts/history?page=1&page_size=20
...
```json
{
  "alerts": [
    {
      "alert_id": "alt-f1e2d3c4",
      "severity": "CRITICAL",
      "title": "[CRITICAL] 熔断触发 - 子公司 A",
      "source": "engine.subsidiary-a",
      "detail": "渗透引擎 LLM 推理延迟 P99=12s，超过阈值 10s，触发紧急熔断",
      "status": "unacknowledged",
      "channels_used": ["im", "email"],
      "created_at": "2026-07-07T10:00:00Z",
...
| 渠道 | 协议 | Payload 格式 | 配置说明 |
|------|------|-------------|----------|
| **邮件** | SMTP | HTML + 纯文本（Multipart） | SMTP 服务器、收件人列表、发件人地址 |
| **即时通讯** | Webhook | Markdown 消息卡片 | Webhook URL、@人员配置（企微/钉钉/飞书） |
| **通用 Webhook** | HTTP POST | JSON（标准模板） | Webhook URL、签名密钥（HMAC-SHA256） |
| **平台内通知** | WebSocket/SSE | JSON（站内消息） | 通过 §4.2 协作平台消息推送，免打扰时段可配置 |

**通用 Webhook Payload 示例**：
...
```json
{
  ...
  "alert_id": "alt-f1e2d3c4",
  "severity": "CRITICAL",
  "title": "[CRITICAL] 子公司 A 触发紧急熔断",
  "detail": "渗透引擎 LLM 推理延迟 P99=12s，超过阈值 10s",
  "source": "engine.subsidiary-a",
  "trace_id": "trace-x9y0z1",
  "timestamp": "2026-07-07T10:00:00Z",
  "ack_url": "https://platform.example.com/api/v1/alerts/alt-f1e2d3c4/acknowledge"
}
```

**通知降级策略**：当所有通知渠道投递失败时，告警升级至上一级处理人，并每 15 分钟重试一次，最多重试 12 次后写入失败日志。

**告警规则禁用行为**：正在被禁用规则所触发的活动告警（即创建规则时处于告警触发中但尚未投递完成的告警），在当前投递完成后不再继续跟踪；已确认但未关闭的告警保持确认状态不变。禁用规则不会删除已有告警历史记录。

### 4.5 第三方 API 集成接口（适配器模式）

**功能描述**：
系统通过统一适配器层集成第三方安全服务 API，扩展资产发现、漏洞扫描与合规检查能力。所有适配器统一采用 RESTful 风格接口，由适配器中间层封装第三方 API 差异。

**架构说明**：

```
子系统（任务调度） → REST API 调用 → 适配器中间层 → 适配器插件 → 第三方 API（Shodan/Qualys/等保工具）
```

**认证方式**：适配器中间层统一接受 OAuth 2.0 Client Credentials Grant 认证（Token 获取同 §4.1.12），适配器插件各自封装第三方认证差异。

**适配器端点总览**：

| 接口 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 资产发现 | `POST` | `/api/v1/adapters/assets/discover` | 在指定网段内发现资产 |
| 漏洞扫描 | `POST` | `/api/v1/adapters/vulns/scan` | 对指定目标执行漏洞扫描 |
| 合规检查 | `POST` | `/api/v1/adapters/compliance/check` | 检查指定资产是否符合合规标准 |
| 查询扫描结果 | `GET` | `/api/v1/adapters/vulns/scan/{scan_id}` | 获取异步扫描任务的结果 |
| 查询适配器状态 | `GET` | `/api/v1/adapters/status` | 查询各适配器连接状态与健康情况 |
| 注册适配器 | `POST` | `/api/v1/adapters/register` | 注册新的第三方适配器插件 |
...
```json
POST /api/v1/adapters/assets/discover
Content-Type: application/json
Authorization: Bearer {token}

{
  ...
  "callback_url": "https://subsidiary-a.com/api/v1/callbacks/adapter"
}
```
  ...
  "assets": [
    {
      ...
      "os": "Linux 5.4",
      "services": ["nginx/1.20", "OpenSSH_8.0"],
      "tags": ["web_server", "dmz"]
    }
  ],
  "total_found": 15,
...
```json
POST /api/v1/adapters/vulns/scan
Content-Type: application/json
Authorization: Bearer {token}
...
```json
{
  "scan_id": "scan-e5f6g7h8",
  "adapter": "nessus",
  "status": "running",
  "progress_percent": 30,
  "estimated_completion": "2026-07-07T11:00:00Z"
}
```
...
```
GET /api/v1/adapters/vulns/scan/scan-e5f6g7h8
Authorization: Bearer {token}
```
...
```json
{
  ...
  "vulnerabilities": [
    {
      "vuln_id": "ext-vuln-001",
      "cve_id": "CVE-2024-XXXX",
      "title": "Apache Log4j 远程代码执行",
      "cvss_score": 9.8,
      "severity": "CRITICAL",
      "affected_target": "192.168.1.100:8080",
      "remediation": "升级 Log4j 至 2.17.1+"
    }
  ],
...
```json
POST /api/v1/adapters/compliance/check
Content-Type: application/json
Authorization: Bearer {token}

{
  ...
  "asset": {
    ...
    "hostname": "web-server-01",
    "os": "Linux 5.4",
    "services": ["nginx/1.20", "PostgreSQL 15"]
  },
  "standard": "等保2.0三级",
  "callback_url": "https://subsidiary-a.com/api/v1/callbacks/adapter"
}
  ...
  "items": [
    {
      "requirement": "身份鉴别",
      "result": "pass",
      "detail": "已启用密码复杂度策略和登录失败锁定"
    },
    {
      "requirement": "访问控制",
      "result": "fail",
      "detail": "存在默认共享账号 guest，建议禁用或删除",
      "remediation": "执行 `userdel guest` 删除默认账号"
    },
    {
      "requirement": "入侵防范",
      "result": "pass",
...

{
  ...
  "auth_type": "api_key",
  "auth_config": {
    "api_key_ref": "vault://adapters/shodan/api_key"
  },
  "options": {
    "timeout_seconds": 120,
    "max_results": 5000
...
```json
{
  "adapter_id": "adap-d4e5f6g7",
  "name": "shodan_assets",
  "type": "asset_discovery",
  "status": "registered",
  "test_connection": "success",
  "registered_at": "2026-07-07T12:00:00Z"
}
```
...
| 错误场景 | HTTP 状态码 | 处理方式 |
|----------|:-----------:|----------|
| 第三方 API 超时 | 504 | 重试 3 次，指数退避（5s→10s→20s），超限返回 `THIRD_PARTY_TIMEOUT` |
| 第三方 API 限流 | 429 | 解析 `Retry-After` 头等待后重试，最多等待 60s |
| 第三方 API 不可用 | 503 | 标记适配器为 `degraded`，返回缓存结果（如有），进入降级模式 |
| 认证失败 | 401 | 自动刷新凭证，刷新失败后标记适配器为 `unhealthy` |
| 数据格式异常 | 422 | 记录原始响应到审计日志，返回部分结果 + 解析错误详情 |
...
**通知过期行为**：`expires_at` 字段标识通知的有效期。过期后通知自动从待办列表中移除（子公司侧不可再拉取到该通知），集团病例库保留审计轨迹。子公司应在收到通知后在有效期内完成处理；过期未处理的通知可联系集团管理员重新推送。
...
| 数据类型 | 传输加密 | 存储加密 | 密钥管理 |
|----------|----------|----------|----------|
| 病例数据 | TLS 1.3 | AES-256-GCM | KMS |
| 漏洞信息 | TLS 1.3 | AES-256-GCM | KMS |
| 渗透记录 | TLS 1.3 | AES-256-GCM | KMS |
| 审计日志 | TLS 1.3 | AES-256-GCM | KMS |
| 用户凭证 | TLS 1.3 | bcrypt（不可逆哈希） | - |
| 集团↔子公司 API 直连 | TLS 1.3（mTLS） | - | 内部 CA + 证书管理 |
| 子公司↔渗透引擎 API | TLS 1.3（mTLS） | - | 内部 CA + 证书管理 |
| 管理平台↔协作平台 WebSocket | TLS 1.3（mTLS） | - | 内部 CA + 证书管理 |
| 智能体通信 | TLS 1.3（mTLS） | - | 内部 CA + 证书管理 |
...
| **数据加密密钥** | 病例数据、渗透记录、审计日志的 AES-256-GCM 存储加密 | 使用 KMS 主密钥加密的数据加密密钥（DEK）模式，数据加密密钥定期轮换 | 2 年 | KMS（AWS KMS / HashiCorp Vault / 等效方案） |
...
| P1 | 数据加密存储 | TLS 1.3 传输加密、AES-256 存储加密 |
