...
> 版本上下文：当前分支 `feature/web-vue3`（web 为 Vue3 重写版）；daemon 侧评估报告 P0.1–P1.16 已全部落地（`docs/2026-08-24/01-daemon-evaluation-report.md`）；**server 侧 P0.1–P0.10 已落地。注：P0.1/P0.2 修复（b230648/2670d0f）当时只提交推送到了 `feature/web-vue3`，未合并回 main，导致 main 工作区一度复核为「失实」；2026-08-30 经 merge（4fdf1f9）补入 main 并与 P0.3/P0.4 一并复验（33/33 文件 295 用例全绿）；P0.5 于同日落地后复验 299 用例全绿；P0.6/P0.7 同日落地复验 301 用例全绿；P0.8 同日落地复验 34/34 文件 306 用例全绿；P0.9 于 2026-08-31 落地复验 34/34 文件 307 用例全绿；P0.10 于 2026-08-31 落地复验 35/35 文件 310 用例全绿；P0.11 于 2026-08-31 落地复验 35/35 文件 312 用例全绿；P1.12 于 2026-08-31 落地复验 36/36 文件 320 用例全绿；P1.13 于 2026-08-31 落地复验 36/36 文件 323 用例全绿；P1.14 于 2026-08-31 落地复验 37/37 文件 331 用例全绿；P1.15 于 2026-08-31 落地复验 38/38 文件 345 用例全绿；P1.16 于 2026-08-31 落地复验 38/38 文件 349 用例全绿；P1.17 于 2026-08-31 落地复验 38/38 文件 354 用例全绿；P1.18 于 2026-08-31 落地复验 38/38 文件 354 用例全绿；P1.19 于 2026-09-01 落地（纯部署配置：compose 删幽灵挂载 + Dockerfile 补拷迁移入镜像，无 server 代码变更，validate-compose 全断言过 + 镜像布局模拟迁移 no-op）；P1.20 于 2026-09-01 落地复验 server 39/39 文件 364 用例全绿、daemon 40/40 文件 372 用例全绿；P1.21 于 2026-09-01 落地（server JSON 应用层 ping 30s 周期广播，web/daemon 零改动，server 全量 39/39 文件 364 用例绿）；P1.22 于 2026-09-02 落地复验 server 39/39 文件 372 用例全绿；P1.23 于 2026-09-02 落地复验 server 39/39 文件 389 用例全绿 + daemon 40/40 文件 372 用例全绿 + 行为探针 ALL PASS**；**P1.24 于 2026-09-02 落地复验 server 40/40 文件 397 用例全绿 + daemon 41/41 文件 381 用例全绿**；**P1.25 于 2026-09-02 落地复验 server 40/40 文件 397 用例全绿 + web 5/5 文件 43 用例绿（daemon 零改动）**；**P1.26 于 2026-09-03 落地复验 server 41/41 文件 407 用例全绿 + web 43 用例绿（见 §4 P1.26）**；**P1.27 于 2026-09-03 落地复验 server 42/42 文件 416 用例绿 + 双实例行为探针 6/6 ALL PASS（见 §4 P1.27）**；**P1.28 于 2026-09-03 落地复验 server 48/48 文件 486 用例绿 ×2 + 四端 tsc（见 §4 P1.28）**
...
| 中 | `db/drizzle-pg.ts`、`db/schema.ts` | ~~整条 drizzle 链死重：`schema.ts` 全仓无运行时引用，查询全走 postgres.js 裸 SQL~~ **✅ 2026-08-31 已修（P1.18，依赖与两文件整体删除）** | ~~空转依赖 + 两份 schema 真相~~（已定性「最差中间态」未处置）已消除，schema 唯一真相收敛为 SQL migrations |
...
| 中高 | `ws/handler.ts:553-558` | ~~channel 信封对 `daemonClients` 无条件全发（含私有频道/DM）~~ **✅ 2026-09-02 已修（P1.22，信封带 allowedDaemonUserIds，daemon 按 agent 成员归属用户定向；公开频道保持全发，解析失败 fail-closed）** | ~~所有用户 daemon 收到他人私有频道明文~~ 已收敛 |
| 中高 | `db/migrate.ts:27-36` | ~~迁移非事务、无 advisory lock（另见 P0.10）~~ **✅ 2026-08-31 已修（P0.10，会话锁 + 每文件事务）** | ~~多实例并发启动失败；中途失败留半态~~ |
| 中 | ~~`docker-compose.yml:28`~~ | ~~挂载不存在的 `src/db/schema.sql` 到 initdb~~ **✅ 2026-09-01 已修（P1.19，删幽灵挂载；并核实出单轨在 Docker 内其实也断——镜像 dist 无迁移 SQL（tsc 不拷非 TS + deploy 阶段 `rm -rf /app/src`），Dockerfile 运行阶段补拷 `src/db/migrations → dist/db/migrations`，全新 compose 部署从「启动建表 ENOENT 崩溃」变可用）** | ~~幽灵文件；双轨初始化已断裂成单轨仍误导排障~~ 已消除 |
| 中 | `db/schema.ts` vs migrations | ~~drizzle 模型系统性漂移（无 `client_nonce`、索引口径、无 TZ、缺 dispatches 等）~~ **✅ 2026-08-31 已修（P1.18，drizzle 链整体删除，漂移随删除不复存在）** | ~~两份 schema 真相；drizzle-kit 生成迁移会灾难~~ |
| 中 | `pubsub.ts:97-102` | Redis 故障 publish 静默吞错仅一次日志 | 多实例下 Valkey 抖动 → 实例间互达中断且无告警 |
| 中 | `pubsub.ts` + `handler.ts:276,282` | ~~全部 WS 事件共用单频道扇出，terminal-frame 每帧 PUBLISH 到所有实例~~ **✅ 2026-09-02 已修（P1.22，广播面/定向面分频道——`slock:ws:v1:channel`/`:all` 全局订阅 + `slock:ws:v1:u:<userId>` 随本地连接动态订/退；顺手修 Redis 回环双投：PUBLISH 会回环给发布者自身订阅连接，加 `__mid` 实例源标记去重）** | ~~多实例 pubsub 流量 = 全局事件量 × 实例数~~ 已消除（多实例滚动升级需同批重启，新旧频道名不兼容） |
| 中 | `ws/handler.ts:533-541` | ~~deliver 裸 send 不查 `bufferedAmount`~~ **✅ 2026-09-02 已修（P1.22，deliver() 收口全部发送 + 超 4MB terminate 强制重连走 REST 补拉，wsSlowConsumerTerminated 计数器）** | ~~慢消费者无背压，高频帧内存无限积压~~ 已消除 |
...
| 中高 | `reminder-scheduler.ts:70-73、117-134` | ~~认领不校验目标 owner daemon 在线；`sendToDaemon` daemon 分支找不到连接静默丢弃~~ **✅ 2026-09-02 已修（P1.23，认领 SQL 门控 `a.user_id = ANY(本地 daemon 用户)`，离线行保持 scheduled 等重连，不占 LIMIT 名额防饥饿）** | ~~daemon 离线 → 提醒被标 fired 但无人接收，一次性提醒永久丢失~~ 已消除（系统性丢失；认领→投递毫秒级窗口另见下行） |
| 中 | `reminder-scheduler.ts:98-104` | 认领（标 fired）与投递间有崩溃窗口，at-most-once，无 ack | ~~server 重启丢已认领未投递提醒，不可察觉~~ **P1.23 取舍立此存照：系统性丢失（离线 daemon 全量静默丢弃）已由认领门控消除；认领提交到 socket 发送之间的毫秒级窗口仍在（需 daemon ack 协议才可消除，归两阶段 claimed→delivered 未来项）** |
...
| 中 | `lib/reminders.ts:24-26`、`reminder-scheduler.ts:137` | ~~`daily@HH:MM` 依赖 server 本地时区；周期重排以处理时刻为基准逐轮漂移~~ **✅ 2026-09-02 已修（P1.23，`reminders.timezone` 存 IANA 名（021 迁移），daily@HH:MM 走 Intl 按 tz 计算；重排改 `nextFireNoDrift` 以原 fire_at 为基准等差快进/逐日推进）** | ~~UTC 部署时东八区用户提醒错 8 小时；every:1h 实际 >1h~~ 已消除 |
| 中 | `people.ts:135` + daemon `agent-cost-tracker.ts:219` | ~~daemon 成本记账纯本地 JSON，无上报通道；metrics_samples 无 cost 列；`costUsd: null` 硬编码~~ **✅ 2026-09-02 已修（P1.24，`/api/agent-costs/sync` 绝对值同步单调 upsert + `agent_cost_daily` 表 + people stats agent/human 两口径接真数据；metrics_samples 未加 cost 列——专用表已覆盖观测需求，取舍见 P1.24）** | ~~server/Web 侧成本观测完全断链~~ 已打通（daemon 本地 `slock cost show` 不受影响） |
| 中 | `agent-duty.ts:13-15`、`metrics.ts:10-12` | ~~presence/在线数基于本实例 `daemonClients` Map，HTTP 读路径未跨实例~~ **✅ 2026-09-03 已修（P1.27，`lib/presence.ts` Redis 在线并集：`computerOnlineFor`/`computerStatusPayload`/metrics 在线计数全部改查「本地 ∪ 全部实例」缓存；未配 VALKEY_URL 退纯本地，行为不变）** | ~~多实例下 daemon 连在实例 B 时实例 A 系统性显示离线~~ 已消除（双实例探针实锤：daemon 连 A，B 的 /api/daemon/status 6s 内 connected=true） |
...
| 中 | `lib/notifications.ts:5` vs 3 处调用点 | `"dm"` 通知类型 P1.25 起已产生（DM 分支事务外 best-effort createNotification，见 §4 P1.25）；`"reminder"` 类型仍未产生，归后续 | DM 通知缺口已补；普通提醒仍无通知中心通知 |
| 中 | ~~`agents-dispatch.ts:162-297`~~ | ~~dispatch 状态机无 completed 终态、无验收端点；reported 永挂，与看板卡片不同步~~ **✅ 2026-09-03 已修（P1.26：completed 终态 + accept 验收端点 + 卡片↔台账双向同步 + 离线黑洞告警，见 §4 P1.26）** | ~~任务合同无闭环~~ 合同已闭环 |
| 中低 | `metrics.ts:51-69` | ~~restoreCounters 仅恢复 5/10 计数器，patrol/machineAuth* 重启清零~~ **✅ 2026-09-03 已修（P1.27，024 迁移补 7 个计数器列 + instance 列，restoreCounters 全量恢复 12/12 并按本实例最新行取，legacy 回退保升级连续性）** | ~~重启后观测归零、累计有静默缺口~~ 已消除 |
| 中低 | `ws/handler.ts:198-219` | runtimes/last_ready_at 仅连接时上报，长连接不刷新 | 装机变化需重连可见；活性语义失真 |
| 中低 | `002_notifications.sql` | notifications 无 TTL 清理 **✅ 2026-09-02 已修（P1.25：purgeReadNotifications 挂 metrics-persist tick，仅清已读超 30 天，未读恒不自动清——见 §4 P1.25）**、无删除端点（未做，归 P2） | 通知表无限增长 → 已读行 30 天 TTL 兜底 |
| 低 | scheduler cleanup 未在 shutdown 调用；fired 提醒 snooze 后永不触发（僵尸端点掩盖）；已读不广播多端不同步 **✅ P1.25（notification.read 广播，见 §4）**；notifications 测试无数据时静默 return（假绿风险）**✅ P1.25（notifications.test.ts 数据驱动重写，真实 DM 链路造数据）** | — | 见 §4 P2 |

**后台机制状态**：Reminder 调度（闭环·单实例；P1.23 起认领前校验 owner daemon 在线、重排锚定原 fire_at 消漂移、daily@HH:MM 按 IANA 时区）/ 通知链路（闭环·P1.25 起 DM `dm` 通知接线 + 已读广播 `notification.read` 多端同步 + 已读行 30 天 TTL 清理；`reminder` 通知类型仍未产生、通知 REST 列表蛇形列名 vs WS camelCase 的契约债归后续）/ Metrics 计数（闭环·P1.27 起 presence 读路径跨实例）/ 采样持久化（P1.27 起 instance 标识 + 全计数器列；cost 列仍无——专用表已覆盖，取舍见 P1.24）/ 成本落库（闭环·P1.24 起 daemon 60s 绝对值同步 → `/api/agent-costs/sync` 幂等单调 upsert 落 `agent_cost_daily`） Runtime probe（闭环·连接时点）/ WS 心跳（闭环）/ T8 分诊（闭环）/ duty（闭环）/ Dispatch（闭环：P1.26 起 completed 终态 + 经理验收端点 + 卡片↔台账双向同步 + 离线黑洞告警；取号竞态 P0.5 已修）/ 任务看板（部分：claim 竞态、无归属校验）。
...
- **零覆盖（按风险排序）**：~~① `agents-messages.ts`（327 行 12 端点，agent 侧消息面全盲）~~（✅ 2026-09-03 P1.28 补 15 用例）；~~② `preview.ts`（SSRF 面）+ `integrations.ts`~~（✅ P1.28 补 9 用例）；~~③ `agents-credentials.ts`（凭据生命周期，安全敏感）~~（✅ P1.28 补 7 用例）；~~④ `lib/reminder-scheduler.ts`（194 行，tick 逻辑仅手动 E2E）~~（✅ P1.28 补 8 用例）；~~⑤ `lib/rate-limit.ts`（全局 hook 无回归网）~~（✅ P1.28 补 7 用例）；⑥ orgs invites（仍零覆盖）；⑦ actions.ts（P0.8 已补 5 用例，此处为报告原文未勾）；~~⑧ `db/migrate.ts`（坏一次全员启动失败）~~（2026-08-31 P0.10 已补 3 用例）。
...
| 2 | P1 | web `ForgotPasswordPage.vue` ↔ server `auth.ts` | ~~`/api/auth/forgot-password`、`/reset-password` 不存在（CSRF 豁免已预留）~~ **✅ 2026-09-01 已修（P1.20 补齐，见 §4 P1.20）** | ~~忘记密码页提交必 404，找回通道不可用~~ 已补齐 |
| 3 | P1 | daemon ~~`cli/action.ts:18`~~ ↔ server `actions.ts:6` | ~~CLI 打 `/internal/agent/:agentId/prepare-action`，server 只有 `/api/actions/prepare`~~ **✅ 2026-09-01 已修（P1.20 砍除：action_cards 按 P0.8 冻结定调不新增 agent 面，CLI 命令删除）** | ~~agent 侧操作卡片链路 404~~ 命令已下线 |
| 4 | P1 | daemon ~~`cli/integration.ts:11,25`~~ | ~~CLI 打 `/internal/agent/:agentId/integrations(/login)`，server 无 agent 面集成端点~~ **✅ 2026-09-01 已修（P1.20 砍除：integrations 本为假成功 stub，CLI 命令删除）** | ~~agent 集成登录 404/401~~ 命令已下线 |
| 5 | P1 | daemon ~~`client.ts:22-63`~~ | ~~14 条重写规则指向不存在的 `/internal/agent-api/*` surface（仅 managed/self-hosted-runner 模式激活，当前 spawn 链路不触发）~~ **✅ 2026-09-01 已修（P1.20 重写面删除，clientMode 字段保留自省展示用）** | ~~潜伏地雷：启用 proxy/credential auth 模式时 agent REST 全 404~~ 已拆除 |
| 6 | P1 | web ~~`wsManager.ts:67,92-98`~~ ↔ server ~~`handler.ts:601-647`~~ | ~~web 看门狗 70s 无 onmessage 即重连；server 心跳是协议层 ping（不触发 onmessage），从无 JSON ping~~ **✅ 2026-09-01 已修（P1.21，server 每 30s 对 browser 连接发 JSON 应用层 ping + 连接建立即发一条；web 看门狗从此被喂上，回 pong 由 server 容忍）** | ~~空闲时段 ~70s 强制重连，每次触发 backfillAll+flushAllPending → 重连风暴 + 冗余补拉~~ 已消除 |
...
| P1.18 | ~~处置 drizzle 死链：移除 `drizzle-orm`+`drizzle-pg.ts`+`schema.ts`（413 行），或真正迁移——不要停在中间态~~ **✅ 2026-08-31 已修**：选择「移除」路线——全仓 grep 确认 `drizzle-pg.ts`（drizzle-orm/pg-core 再导出垫片）与 `schema.ts`（纯 pgTable 表定义）仅互相引用成死闭环、无其他运行时调用方（查询全走 postgres.js 裸 SQL；无 drizzle.config / drizzle-kit scripts）。删除两文件（含注释约 413 行）+ 清理 dist 陈旧产物；`package.json` dependencies 移除 `drizzle-orm`，`pnpm install` 同步 lockfile（Packages: -1，lockfile 零 drizzle 残留）。**schema 真相立此存照：此后唯一真相是 `db/migrations/*.sql`**，「两份 schema 真相」与 drizzle-kit 灾难面随删除消除；docker-compose 的 `schema.sql` 幽灵挂载另属 P1.19 不在本项。纯死码移除无行为变更、无新增用例；复验：`tsc --noEmit` 零错误、无 drizzle 依赖下 server 正常启动（`/api/health` = `{db:true}`）、全量 38/38 文件 354 用例绿 | `db/drizzle-pg.ts`（删）+ `db/schema.ts`（删）+ `package.json` + `pnpm-lock.yaml` | 消除空转依赖 + 两份 schema 真相，schema of record 收敛为 SQL migrations |
| P1.19 | ~~删除/补齐 docker-compose 的 `schema.sql` 幽灵挂载~~ **✅ 2026-09-01 已修**：① `docker-compose.yml` 删幽灵挂载行（`src/db/schema.sql` 不存在；挂载缺失宿主路径时 Docker 会在源码树创建同名空目录、initdb 撞目录即失败），原位留注释立此存照「schema 唯一真相 `db/migrations/*.sql`（P1.18），server 启动 `runMigrations()` 自动建表，postgres 只需空库」；② 顺手核实「单轨初始化」在 Docker 内的真实可用性——**发现镜像内根本没有迁移 SQL**：`tsc` 不拷非 TS 文件、Dockerfile deploy 阶段又 `rm -rf /app/src`，`runMigrations` 按运行时 `__dirname`(=dist/db) 相对解析必 ENOENT，全新 `docker compose up` 实为 server 启动即崩（比幽灵挂载更深的断点）；修复：Dockerfile 运行阶段 `COPY --from=build /repo/packages/server/src/db/migrations ./dist/db/migrations`——仍单轨、不复活 schema.sql 双真相；③ 验证：`node scripts/validate-compose.mjs` 全断言通过（CI L1 同款静态校验，本机无 docker CLI 的替代手段）+ 本地模拟镜像布局（迁移拷入 `dist/db/migrations`）跑编译产物 `node dist/db/migrate.js` → `[DB] All migrations up to date`（no-op，证实 dist 相对解析可用）；无 server 代码变更，tsc/vitest 覆盖面不受影响。**未验项立此存照**：docker 内真实 build/up 未跑（本机无 docker CLI），首次 compose 部署时关注 server 启动日志出现 `[DB] Running migration` | `docker-compose.yml:26-32` + `packages/server/Dockerfile` | 消除幽灵挂载 + 修复镜像内迁移缺失（全新部署从必崩变可用） |
| P1.20 | ~~契约缺失端点补齐或砍掉：forgot/reset-password；`prepare-action`、`integrations` agent 面（或改 CLI 指向）；删 `/internal/agent-api` 重写面~~ **✅ 2026-09-01 已修**：① **forgot/reset-password 补齐**（drift #2）——按 web `ForgotPasswordPage.vue` 既有契约（`{message, devCode?}`）实现 `POST /api/auth/forgot-password` + `/api/auth/reset-password`；仓库无邮件基建，完整验证码流仅在 `SLOCK_DEV_RESET_CODE=1` 显式开启（devCode 回传响应，同 P1.17 `SLOCK_DEV_TOKEN` 模式：`collectInsecureConfig` 标记、生产无 ALLOW_INSECURE 启动即拒），关闭态恒返回诚实文案、不落任何状态（无假成功、不查库无枚举）；实现要点：启用 000_canonical 预留的死列 `users.reset_code/reset_expires`（020 迁移把 reset_code VARCHAR(10)→VARCHAR(64)，存 sha256 而非明文）+ 6 位 CSPRNG 拒绝采样码 + 时序安全比对 + 10min TTL + 单次使用（消费/改密/token_version 轮换单条条件 UPDATE 原子完成，并发重放仅一成功）+ 成功后全会话吊销（对齐 change-password 语义）；CSRF 豁免名单（index.ts:230）regex `forgot|reset` 前缀本就覆盖，web 零改动。② **prepare-action / integrations agent 面砍除**（drift #3/#4）——action_cards 按 P0.8「半成品冻结、不新增端点」定调不给冻结功能建 agent 面，integrations 本身是纯 stub（`POST /login` 恒 200 假成功），且两 CLI 命令（`slock action prepare`、`slock integration list/login`）打的端点从未存在过（命令从未可用、砍除零损失）：删 `cli/action.ts`、`cli/integration.ts` + `cli.ts` 两 import 两注册。③ **删 `/internal/agent-api` 重写面**（drift #5）——`client.ts` 删 `usesAgentApiSurface` + `rewriteAgentCredentialPath`（14 条规则指向 server 上从未存在的 surface，managed-runner/self-hosted-runner 模式一旦激活 agent REST 全 404 的潜伏地雷；两模式当前 spawn 链路不触发、重写面从未生效过），`clientMode` 字段保留（cli auth 自省展示用）。④ 验证：server `tsc` 零错误 + 全量 **39/39 文件 364 用例绿**（新增 `test/password-reset.test.ts` 8 用例：纯函数 4 离线 + 关闭态负路径 4 在线含 CSRF 豁免回归；`config.test.ts` +2 标记用例）+ daemon `tsc` 零错误 + **40/40 文件 372 用例绿** + 迁移 020 随 `NODE_ENV=test` server 启动在真库应用（`[DB] Migration applied: 020_password_reset_code.sql`）。行为探针（PORT=3002 + `SLOCK_DEV_RESET_CODE=1`）：register → forgot 返 devCode → reset `{ok:true}` → 同码重放通用 400（单次使用）→ 旧密 401/新密 200 → 旧 access token 401（token_version 轮换生效）；生产拦截探针：`NODE_ENV=production` + 开关启动 exit(1) 且唯一标记项即 `SLOCK_DEV_RESET_CODE`。**取舍立此存照**：dev 开启下 devCode 有无可探测邮箱存在性（开关本身即不安全模式，已入 collectInsecureConfig）；生产无邮件基建时找回通道 =「联系管理员」诚实文案，未来接入邮件仅需替换 devCode 交付通道 | `routes/auth.ts` + `lib/password-reset.ts`（新）+ `db/migrations/020`（新）+ `lib/config.ts` + daemon `cli.ts`/`client.ts` + 删 `cli/action.ts`、`cli/integration.ts` | 消除「页提交必 404」死契约 + 三个潜伏 404 面；找回密码 dev 全链路可用、生产诚实降级 |
| P1.21 | ~~server 每 30s 加一条 JSON `{type:"ping"}` 广播（或 web 拉长 watchdog），消除 70s 重连风暴~~ **✅ 2026-09-01 已修**：选择报告首选方案（server 侧 JSON ping，web 看门狗 70s 阈值保持原语义不弱化）。**现场发现两端早已就绪、缺口极小**：web `wsManager.ts:132-134` 已特判 `type==="ping"` 回 `{type:"pong"}` 并喂狗（任何 onmessage 都 resetWatchdog），`wsManager.test.ts:80/115` 已有 ping→pong 与看门狗重置回归；shared `WsFromBrowserMessage` 已预留 `{type:"pong"}`；server browser 消息 handler（handler.ts:317）已容忍 `pong`。缺的仅是 server 从不发 JSON ping。① **shared**（先改 shared 纪律）：`WsToBrowserMessage` union 加 `| {type:"ping"}`（纯增量；`WsChannelBroadcast` Extract 子集不受影响）；② **server `ws/handler.ts`**：30s 心跳 interval 对所有 browser 连接周期发 JSON ping（仅 browser 面——daemon 侧 `WsToDaemonMessage` 无 ping 类型，daemon 无重连风暴问题；协议层 heartbeatPing 清死连接语义不变）。③ **web/daemon 零改动**（pong 应答与喂狗逻辑原位生效）。**方案迭代留痕**：初版曾加「连接建立即发一条 ping」（兼作在线测试快速断言点），实测打破 ws.test.ts 既有用例「`connected` 后第一条为业务消息」的 nextMessage 序列契约（4 用例失败），已撤回——看门狗首口由 `onopen` 喂，周期 ping 才是核心；连带把 `nextMessage` helper 改为收到 ping 自动回 pong 并透明跳过（长跑套件连接跨 30s tick 不再让序位/负向断言误吃 ping 帧——实测修复了 DM 负向断言撞 tick 的 flaky）。验证：shared/daemon/web/server 四端 `tsc` 零错误（shared union 增量不破坏三端穷举 switch）；server 全量 **39/39 文件 364 用例绿**（含 ws.test.ts 15 用例，helper 透明化后既有用例零改动）；web 套件回归绿（shared 类型变更）；行为探针：65s 窗口内观察到连接后 ≥2 跳 `{type:"ping"}`、间隔 ≈30s、server 容忍探针 pong。**未验项立此存照**：「空闲 70s 不再重连」的端到端体感需长时观察（或部署后看 web 控制台不再出现 70s 周期 reconnect 日志） | `packages/shared/src/index.ts` + `packages/server/src/ws/handler.ts` + `test/ws.test.ts`（helper 透明化） | 消除空闲 ~70s 重连风暴（backfillAll 冗余补拉随之消失） |
| P1.22 | ~~WS 收敛：`handleEnvelope` daemon 全发改按频道 agent 成员定向；deliver 查 `bufferedAmount` 做背压；pubsub 按事件类型/userId 分频道~~ **✅ 2026-09-02 已修**：① **daemon 定向**——`broadcast()` 对 private/dm 在人类成员解析后追加 agent 成员归属解析（`channel_members cm JOIN agents a ON cm.member_id = a.id AND member_type='agent'` → `DISTINCT a.user_id`；daemonClients 按 owner userId 记账），信封新增 `allowedDaemonUserIds`，`handleEnvelope` 据此过滤 daemon 投递——公开频道保持全发（@提及自动入圈依赖广播面）、私有频道/DM 仅 agent 成员的 owner daemon 收到（人类成员 daemon 不再收他人私有频道明文）；agent-owner 查询失败整体丢弃（与 P0.2 同 fail-closed，可按 seq 游标 REST 补拉）。② **deliver 背压**——`deliver()` 收口 `handleEnvelope` 全部 socket 发送（此前 channel/daemon/all-daemons 三处裸 send 绕过），`bufferedAmount` 超 4MB → terminate 强制重连走 REST 补拉（静默跳过会丢消息且不可察觉，断开重连才是可恢复路径），新增 `wsSlowConsumerTerminated` 计数器。③ **pubsub 分频道**——广播面 `slock:ws:v1:channel` / `slock:ws:v1:all` 全局订阅；定向面 `slock:ws:v1:u:<userId>`（承载 user/daemon/terminal-frame 三类信封）随本地连接增减动态订/退（registerConnection/close 幂等 refresh；setPubSub 对在线用户补订阅）——多实例下高频 terminal-frame 与定向事件不再对全部实例全量扇出。④ **顺手修实锤 bug**——`pubsub.ts` 原注释声称「Redis pub/sub 不会回环给发布者自身」语义不符（PUBLISH 会回环到发布者自身订阅连接，单实例 InProcess 掩盖了这点），Redis 模式下每条消息本地投两次；加实例源标记 `__mid`（instanceId:seq）有界 FIFO 去重，对 access.ts 等其他消费方透明。⑤ 验证：server `tsc` 零错误 + 全量 **39/39 文件 372 用例绿**（`ws-broadcast.test.ts` 8→16 用例：agent 成员定向 ×2 / agent-owner 解析失败 fail-closed / 公开频道 `allowedDaemonUserIds=null` / pubsub 路由频道名四信封 / setPubSub 全局订阅 / deliver 背压三态）+ 行为探针（3001 test server）：daemon `agent:status` → per-user 定向频道 → 同用户浏览器收到、他用户浏览器 2.5s 超时隔离；探针脚本与 `zzt*` 探针用户已清理。**取舍立此存照**：多实例滚动升级时新旧实例频道名不兼容（旧 `slock:ws:v1` 单频道 → 新分频道），需同批重启；daemon 定向只覆盖 agent 成员——若未来出现「人类成员的 daemon 需要频道事件」场景需扩 `allowedDaemonUserIds` 语义 | `ws/handler.ts` + `lib/pubsub.ts` + `lib/metrics.ts` | 消除 daemon 全发私有频道明文 + 慢消费者无界积压 + 多实例 pubsub 全局扇出 + Redis 回环双投 |
| P1.23 | ~~提醒可靠性：认领前校验 owner daemon 在线（或两阶段 claimed→delivered）；`daily@HH:MM` 存 IANA 时区；重排以原 fire_at 为基准消漂移~~ **✅ 2026-09-02 已修**：① **认领门控**（选「校验在线」而非两阶段 ack）——认领 SQL `JOIN agents a ON a.id = r.owner_id` 带出 `owner_user_id` 并加 `a.user_id = ANY(本地 daemonClients 用户)` 过滤：daemon 离线的到期行不认领（status 保持 scheduled 等重连），一次性提醒不再被「标 fired + sendToDaemon 静默丢弃」；SQL 侧过滤而非认领后 JS 丢弃——离线行不占 `LIMIT 20` 名额，不会因一批永久离线 owner 饿死在线 owner；`FOR UPDATE OF r SKIP LOCKED` 只锁 reminders 行不与 agents 锁耦合（**实锤留痕：初版误写 `FOR UPDATE SKIP LOCKED OF r` 语序，PG 语法错每 tick 回滚、全部提醒无法认领，全量测试未抓到（tick 不在测试面）、行为探针当场击中，探针价值实证**）；投递改用 JOIN 带出的 owner_user_id，顺手消一处逐行回查 N+1；多实例下各实例只认领本地有 daemon 连接的行，实例间天然分工无重复投递。② **IANA 时区**——021 迁移 `reminders.timezone VARCHAR(64)`（NULL=存量行回退 server 本地 tz 行为不变）；`nextFireFromRepeat` 加 tz 参数，daily@HH:MM 走 `Intl.DateTimeFormat` 按 tz 换算（挂钟语义，DST 切换日两次逼近修正），顺手修 `daily@25:70` 此前 setHours 静默翻日 → 显式拒绝；POST/PATCH 接 `timezone`（非法 IANA → 400；缺省落 server 本地 tz 显式化），resume 按存储 tz 重排，DTO 回显；daemon（跑在用户机器上）创建时随附本机时区——CLI `slock reminder schedule` 自动带、`--tz` 可覆盖，MCP `schedule_reminder` 加可选 `timezone` 参数——daily@HH:MM 按「用户的钟」触发，server UTC 部署不再错 8 小时。③ **消漂移**——新增 `nextFireNoDrift(repeat, lastFireAt, now, tz)`：以刚触发的原定 fire_at 为基准（此前用处理时刻，every:1h 实际 >1h 逐轮累积），间隔型（every:/hourly/daily）O(1) 等差快进保持锚点序列，daily@ 逐日推进跳过停机积压天（400 天上限兜底退化为下一未来槽位，不放弃提醒）。④ 取舍立此存照：认领提交到 socket 发送间毫秒级崩溃窗口仍在（at-most-once），彻底消除需 daemon ack 两阶段 claimed→delivered 协议，不在本项；人类侧 `/api/reminders`（owner 语义不兼容的僵尸端点）不动，归 §2.5 僵尸端点项。⑤ 验证：server `tsc` 零错误 + 全量 **39/39 文件 389 用例绿**（`lib.test.ts` +14 纯函数用例：tz 换算上海/纽约 DST 双次逼近/旧行为兼容/越界拒绝 + 消漂移四态；`patrol.test.ts` +3 路由用例：tz 入库回显/非法 400/PATCH 改）+ daemon `tsc` 零错误 + **40/40 文件 372 用例绿** + 021 迁移真库应用 + 行为探针 ALL PASS（3001 test server：在线 owner 一次性提醒 fired+WS 收到 reminder.fire / every:30s 重排 fireCount=2 时 fireAt 恰为到期+60s / 离线 owner 保持 scheduled / timezone 入库；探针 `zzp123_` 用户零残留，脚本留存 `scripts/probe-p123-reminder.mjs` 复用） | `lib/reminders.ts` + `db/migrations/021`（新）+ `lib/reminder-scheduler.ts` + `routes/agents-reminders.ts` + daemon `cli/reminder.ts`/`mcp/slock-mcp-server.ts` + `scripts/probe-p123-reminder.mjs`（新） | 消除 daemon 离线一次性提醒永久丢失 + daily@ 时区错 8 小时 + every: 逐轮漂移 |
| P1.24 | ~~打通 daemon→server 成本上报（WS 或 internal REST 上报 `(agent, channel, utcDay, usd)`，与 `createSessionCostDelta` 差值语义对齐），`people.ts:135` 接真数据~~ **✅ 2026-09-02 已修**：① **server 建表**——022 迁移 `agent_cost_daily`（PK (agent_id, channel, day)、cost_usd NUMERIC(14,6)；channel 为 daemon 归一化账本键而非 channels FK——DM 归并 "dm" 无法可靠回链 channel id，存名不存 uuid）；② **上报端点** `POST /api/agent-costs/sync`（machine token 走 authenticate，Bearer 无 CSRF 面；行级归属校验 fail-closed——agentId 优先 / agentName 兜底，解析不到「调用者名下 agent」即 skip 不 400 不泄露存在性；非法行逐条 skip；行上限 200 违者 400）UPSERT `ON CONFLICT … DO UPDATE SET cost_usd = GREATEST(t.cost_usd, EXCLUDED.cost_usd)` 单调收敛——重试 / 乱序 / 账本重置不重复计费、无需 ack 协议；③ **daemon 上报**——新增 `cost-reporter.ts` 包装 ICostTracker（记账照旧 + 标脏 (agent, channel, day)），定时（60s，`SLOCK_COST_REPORT=0` 关）批量上报脏键的**当日累计绝对值**而非逐回合增量流：账本本身已由 createSessionCostDelta 做过「本次−上次」差值（P0.5），server 存的即增量之和而非会话累计原值——差值语义在此意义上对齐；绝对值同步使重试 / 重放天然幂等，flush 期间新回合按值复检保留脏键不漏报，失败保脏下轮重试，零值（PTY 回合）不产生请求，daemon stop 3s 兜底补投；④ **people stats 接真数据**——agent 对端 = 该 agent UTC 日窗口合计、human 对端 = 名下 agents 合计（成本不做「调用者可见频道」过滤：账本按归一化频道名记账无法可靠回链 channel，且 agent 对端在 resolveVisiblePeer 已有 org 门槛——与消息/任务口径差异注释立此存照），无成本行恒 null（web 端 typeof number 才显示 $ 徽标，避免全员 $0.00 噪音）；⑤ 验证：server tsc 零错误 + **40/40 文件 397 用例全绿**（`agent-costs.test.ts` 8 用例：单调收敛 / agentName 兜底 / 他人 agent fail-closed / 非法行 skip / 负载 400 / people stats 两口径 / days 窗口）+ daemon tsc 零错误 + **41/41 文件 381 用例全绿**（`cost-reporter.test.ts` 9 用例）+ 022 迁移真库应用。**实锤留痕**：初版 sync INSERT 列清单含 updated_at 但 VALUES 仅 4 表达式（PG 42601）、people stats 成本查询复用 4 参但 SQL 未全引用（PG 42P18 无法推型），两 bug 均靠真库测试当场击中、修复后全绿。**取舍立此存照**：成本不做可见频道过滤（见 ④）；PTY 零美元回合不落库（成本视图仅在真 USD 出现后有数据）；绝对值同步意味着 daemon 账本被清空后 server 值不回退（GREATEST 保高，防重复计费优先于可缩容） | `db/migrations/022`（新）+ `routes/agent-costs.ts`（新）+ `routes/people.ts` + daemon `cost-reporter.ts`（新）+ `config.ts` + `daemon-core.ts` + `test/agent-costs.test.ts`（新）+ daemon `test/cost-reporter.test.ts`（新） | server/Web 侧成本观测断链已打通（daemon 本地 `slock cost show` 不受影响） |
| P1.25 | ~~通知补齐：DM 产生 `dm` 通知；notifications 过期清理 job（挂 metrics-persist tick）；已读广播 `notification.read`~~ **✅ 2026-09-02 已修**：① **DM 通知**——POST /api/messages/send 的 DM 分支经 dmOtherMembers 取「除发送者外」成员，对端**人类**收方逐个 createNotification（type="dm"，title=`{senderHandle} 给你发来私信`，body=内容截 200；agent 对端无通知中心，靠 daemon WS 唤醒，由 dmAgentRecipients 承担；人机双成员 DM 只通知人类一方），放事务外 best-effort（失败仅 warn 不阻断发送，与 @mention 通知同语义），自言自语（DM 只有自己）自然不通知；agent 发送面（agents-messages.ts）不产生 dm 通知的取舍立此存照（agent 无通知中心，其 owner 视角通知归 reminder 语义，与 P1.23 人类侧 reminders 僵尸端点思路一致）。② **已读广播**——shared WsToBrowserMessage 新增 `{type:"notification.read"; ids: UUID[]|null; all: boolean}`（纯类型增量；web 本地扩展类型同步补 NotificationReadEvent，LocalWsEvent 6→7 事件）。notifications 路由三处标记已读（单条 / 批量 ids / 全部）成功后 sendToUser 广播给该用户全部浏览器连接——多标签页同步，发起端 store 已乐观更新，广播回流幂等（已读行直接跳过、计数不重复减）。web wsDispatch 加 notification.read 分支（all→markAllAsRead，ids→逐条 markAsRead，均幂等）。③ **TTL 清理**——`lib/metrics-persist.ts` 导出 purgeReadNotifications（`DELETE … WHERE read = true AND created_at < now() - 30 days`；未读恒不自动清——未读是用户待办，静默删除等于丢通知；表增长由「已读清理 + 用户全部已读」兜底，unread 部分索引保证计数查询不受表体增长影响），挂 tick best-effort 调用（tick 60s 一次，测试直调函数验证）。④ 验证：server tsc 零错误 + **40/40 文件 397 用例全绿**（notifications.test.ts **数据驱动重写** 7 用例——DM 字段（REST 蛇形列名）/ 收发双方视角 / unreadOnly / 单条已读递减+重复标记幂等+404 / 批量精确递减 / WS notification.new+notification.read 双事件 / 全部已读 all=true / purgeReadNotifications 直调三行插库断言；顺带修 §5「无数据静默 return」假绿风险）+ web vue-tsc 零错误 + **5/5 文件 43 用例绿**（wsDispatch +2：notification.read ids/all 两形态路由与重复广播幂等）+ 四端 tsc 验证（daemon 零改动）。**实锤留痕**：测试初版用一次性 nextMessage 监听，挂载窗口丢「server 在 HTTP 响应**之前**推送的 WS 事件」（notification.new 与 agent:deliver 都在 send 响应前发出），vitest 连败三轮后加原始帧诊断实锤、改**持久收集器**（连接即挂监听 + 队列缓冲 + nextOfType 按类型等待）全绿；初版全量回归忘传 DATABASE_URL（helpers 直连 sql 的 cleanupTestData 撞错误凭据假败 26 文件）——server 黑盒套件须 **server 与 vitest 两边都显式 DATABASE_URL**（P1.24 已知坑复踩）；ws.test.ts nextMessage 对 notification.new 透明跳过（与 ping 同款过滤，DM 投递序位断言不受 P1.25 通知事件影响）。**取舍立此存照**：通知 REST 列表返回 DB 蛇形列名（actor_name/channel_id/created_at）而 WS 载荷 camelCase，两形态并存本次不改（web NotificationBell 实际消费的 title/body/read/metadata/type 字段可用；`createdAt` 在 REST 面实为 created_at——显示层时序文本受影响，归后续契约债）；agent→human DM 不产生通知（agent 无通知中心）；`reminder` 通知类型仍未产生（普通提醒目前走 daemon WS 提醒，通知中心覆盖归后续）。 | `routes/messages.ts` + `routes/notifications.ts` + `lib/metrics-persist.ts` + shared `index.ts` + web `types.ts`/`wsDispatch.ts`/`wsDispatch.test.ts` + `test/notifications.test.ts`（重写）+ `test/ws.test.ts` + `scripts/probe-p125-notifications.mjs`（新） | DM/普通提醒无通知中心通知 → DM 已接线；多端已读不同步 → 已广播；通知表无限增长 → 已读行 30 天 TTL |
| P1.26 | ~~dispatch 增加 `completed` 终态 + 经理验收端点，看板卡片与 dispatches 双向同步； daemon 离线时产生 server 侧可见告警（对齐 dead-letter）~~ **✅ 2026-09-03 已修**：① **completed 终态**——023 迁移放宽 `dispatches_status_check`（约束名经 pg_constraint 实锤）放行 `completed` + 新增 `completed_at`；STATUSES 白名单与 GET 台账返回列同步补齐。② **经理验收端点** `POST /internal/agent/:agentId/dispatch/:dispatchId/accept`——仅 dispatch 经理（from_agent_id）可调、仅接受 reported；**单语句条件更新**（`WHERE … AND status='reported' RETURNING`）保证转换原子，并发双 accept 一中一 404（终态不可再入）；验收联动卡片 in_review→done + recordTaskEvent（detail=accepted）+ 🎉 通知消息给 worker。③ **看板卡片 ↔ dispatches 双向同步**——正向（既有 create→in_progress / report→in_review / cancel→closed + 新增 accept→done）；**回向新增** `lib/dispatch-sync.ts syncDispatchOnCardClose`（卡片 done→dispatch completed、closed→cancelled，条件更新仅作用非终态 open/reported，task_message_id 悬空零影响），接入 tasks.ts /update-status（人类）与 agents-tasks.ts /update-status（agent 侧）——消灭「卡片已关、台账永挂」分叉。④ **离线黑洞告警**——`alarmIfDaemonOffline`：目标 agent owner daemon 不在本实例 daemonClients（`computerOnlineFor`）时向经理 owner 浏览器 `sendToUser` 发 **agent:delivery-dead-letter 同款事件（shared 加可选 `reason:"daemon-offline"`）**，web toast 区分文案（⚠️ 暂未送达 vs ❌ 重试耗尽）；派发/回报/撤回/验收四条投递路径全覆盖（告警收件人恒为经理 owner——合同等待方）；取舍立此存照：多实例下 computerOnlineFor 只覆盖本实例连接（跨实例 presence 归 P1.27）；best-effort（经理 owner 无浏览器在线自然丢弃，与死信一致）；daemon 重连不补拉（connected 分支 no-op）故告警文案明示「需重新派发或人工提醒」。⑤ 验证：server tsc 零错误 + 全量 **41/41 文件 407 用例绿**（新增 `test/agents-dispatch.test.ts` 10 用例：建卡+created 事件+离线告警 WS / 回报+经理侧告警 / worker 越权验收 404 / 验收闭环+accepted 事件 / 重复验收 404 / GET 双视角+completed 过滤+completed_at 回显 / 撤销+卡片 closed / 回向同步 done→completed+closed→cancelled / 非经理 403 / 停班 409+复班）+ web 5/5 文件 43 用例绿 + 四端 tsc（shared 可选字段纯增量；daemon 零改动）+ 023 迁移真库应用 + 行为探针 **20/20 ALL PASS**（`scripts/probe-p126-dispatch.mjs`：A 离线告警帧 / B 台账卡片事件 / C 回报+经理侧告警 / D 验收闭环+重复 404 / E 回向同步 / F GET 过滤回显 / G 负路径；探针用户零残留）。**实锤留痕**：① 初版测试/探针把 channelId 传 dispatch 路由 → `resolveChannel` 仅按名匹配（`WHERE name=$1` 不认 UUID）404 "channel not found"——dispatch 系路由 channel 参数语义是**频道名**（对齐 daemon CLI `--channel`），测试修正后全绿；② `syncDispatchOnCardClose` 初版 `$2` 单参数复用（`status=$2` + `$2='completed'` 字面量比较）触发 **PG 42P08 参数类型推断冲突**（varchar vs text）——与 P1.24 的 42P18 同类，真库测试当场击中，改双布尔参数消歧；③ helpers `cleanupTestData` 补 dispatches 双侧清理（评估 §5 清理缺口实锤：dispatches 对 agents 无 ON DELETE，挂种子频道的 dispatch 会使 agents 删除 FK 失败） | `db/migrations/023`（新）+ `routes/agents-dispatch.ts`（accept + 告警）+ `lib/dispatch-sync.ts`（新）+ `routes/tasks.ts` + `routes/agents-tasks.ts` + shared `index.ts`（reason 可选字段）+ web `lib/wsDispatch.ts`（toast 分支）+ `test/agents-dispatch.test.ts`（新）+ `test/helpers.ts`（cleanup 补 dispatches）+ `scripts/probe-p126-dispatch.mjs`（新） | dispatch 合同从「reported 永挂」到完整闭环（open→reported→**completed**/cancelled）；卡片与台账双向同步；离线黑洞 server 侧可见（对齐 dead-letter） |
| P1.27 | ~~presence/metrics 读路径跨实例化（Redis 在线集合替代本地 Map；metrics_samples 加实例标识列，restoreCounters 全量恢复）~~ **✅ 2026-09-03 已修**：① **跨实例在线注册表 `lib/presence.ts`（新）**——每实例一个 Redis SET 键 `slock:presence:v1:<processKey>`（进程级随机 UUID，成员=本实例 daemon userIds；EXPIRE 45s 随同步循环续期——进程崩溃（kill -9 无 close 事件）残留键靠 TTL 自愈，无永久陈旧条目）；同步循环 3s：SADD 自愈重写本实例成员 + SCAN 全部实例键 SMEMBERS 取并集 → remoteUsers 缓存；写路径（连接/断开）即时生效，跨实例状态传播 ≤3s——presence 是引导性信息（列表徽标/告警预检），秒级陈旧可接受，权威投递仍走本实例 socket + P1.22 per-user pubsub。② **读路径全量接入**——`agent-duty.ts computerOnlineFor`（people/agents-public/agents-dispatch 离线告警预检等 4 个 import 方零改动自动受益）+ `computers.ts computerStatusPayload`（接入页轮询，meta 明细仍本实例视角回退 computers 行）+ `routes/metrics.ts`（online.daemons 改并集并新增 `daemonsLocal` 本地明细对照；history samples 带 instance 列）+ `metrics-persist.ts`（daemon_count/agent_online 改并集口径）。**降级语义**：未配 VALKEY_URL（单实例/测试）→ 纯本地集合行为与旧版完全一致；Redis 故障 → 缓存保持最后已知值不清空（避免抖动期间全线闪离），本地路径不受影响（与 pubsub.ts 同一故障哲学）。reminder-scheduler 认领门控**有意保持本地** daemonClients——各实例只认领本地有连接的行是 P1.23 的实例间分工设计，非读路径缺陷。③ **metrics_samples 实例化（024 迁移）**——`instance TEXT NOT NULL DEFAULT 'default'`（存量行回填）+ 补齐 7 个此前未持久化的计数器列（patrol_posted/silent/auto_paused + machine_auth_bcrypt_scans/hits/rejected + ws_slow_consumer_terminated）+ (instance, sampled_at DESC) 索引；`instanceId()` = `SLOCK_INSTANCE_ID` 或主机名（稳定性优先：同部署槽重启要能找回自己上一世的累计值；多实例部署应逐实例配置，同机多进程共用标识会互相接管累计值——立此存照）。④ **restoreCounters 全量恢复**——12/12 计数器（此前 5/12，patrol/machineAuth*/wsSlow 重启清零）；按**本实例最新行**取，本实例无行时回退全表最新行（024 升级首启的 legacy 接续，多实例下首启接管他实例计数一次，60s 内自有行接管恢复口径——bounded 一次性代价）。⑤ 验证：server tsc 零错误 + 全量 **42/42 文件 416 用例绿**（新增 `test/presence-registry.test.ts` 4 用例：本地模式增删视图 / fake client 跨实例并集+SADD/SREM 落键 / Redis 故障缓存保持+本地不受影响 / no-op cleanup；`metrics.test.ts` +5 用例：12 计数器全量恢复 / legacy 回退 / instanceId 有值 / history 行带 instance / online.daemonsLocal）+ 024 迁移真库应用（schema 逐列实锤）+ **双实例行为探针 6/6 ALL PASS**（`scripts/probe-p127-presence.mjs` + `scripts/run-p127-servers.sh`：A(3001/instA)+B(3002/instB) 同 PG 共 Valkey——A1 daemon 连 A、B 的 /api/daemon/status ≤6s connected=true（旧世界恒 false 的实锤复现点）/ A2 自身视角不变 / B1 B 的 metrics online.daemons=1 且 daemonsLocal=0 / C1 metrics_samples 出现 instA+instB 两实例采样行 / C2 新计数器列可写可读 / D1 断开后 B ≤8s 收敛 connected=false；探针用户零残留）。**实锤留痕**：① presenceRemove 初版不动 remoteUsers（「等下一轮扫描收敛」）——纯本地模式无扫描永不收敛，单测当场击中，改为删除即收敛（他实例真仍持有时下一轮扫描加回）；② restoreCounters 单测初版 pgLike 直接用 postgres.js `sql.unsafe`——其返回值是行数组而非 `{ rows }` 包装，`r.rows.length` 抛 TypeError 被 catch 吞成静默 0（正是 P1.25 批评过的「无数据静默 return」假绿同款），对齐 db/connection pgPlugin 的 `{ rows }` 包装后绿。**取舍立此存照**：SCAN+SMEMBERS 全量并集在实例数大时 O(keys) ——当前规模（≤个位数实例）可忽略，规模化可换 Redis pubsub gossip 或 SUNION 静态键集；在线判定秒级陈旧窗口对 dispatch 离线告警意味着「断开后 ≤3s+ 内的告警可能被压制」——告警语义本就 best-effort，可接受；同机多进程共享 instanceId 时计数器互相接管（presence 无此问题——processKey 进程级随机） | `lib/presence.ts`（新）+ `ws/handler.ts`（连接/断开镜像）+ `lib/agent-duty.ts`（computerOnlineFor 委托）+ `routes/computers.ts` + `routes/metrics.ts` + `lib/metrics.ts`（instanceId + 全量 restore）+ `lib/metrics-persist.ts`（instance 列 + 7 计数器列 + 并集口径）+ `routes/agents-dispatch.ts`（注释）+ `db/migrations/024`（新）+ `index.ts`（startPresenceSync/shutdownPresence）+ `test/presence-registry.test.ts`（新）+ `test/metrics.test.ts` + `scripts/probe-p127-presence.mjs` + `scripts/run-p127-servers.sh`（新） | presence/在线数从「单实例 Map」到跨实例一致（多实例部署不再系统性显示离线）；metrics 采样可区分实例、计数器重启不再清零 |
| P1.28 | ~~补测试盲区：agents-messages（含 requireOwnAgent 越权 403 矩阵）、agents-credentials（签发→使用→吊销生命周期）、preview/integrations、rate-limit、scheduler（假时钟）；WS 入站消息加 zod 运行时校验~~ **✅ 2026-09-03 已修**：① **六个新测试文件**（+70 用例，全量 48/48 文件 486 用例绿，连跑两次确认 tenant 稳定）：`test/agents-messages.test.ts` 15 用例（send 公开/私有/越权/未入圈 + requireOwnAgent 403/404 矩阵 + sk_agent_/sk_machine_ 双认证路径 + edit/delete 硬删软删 + receive 游标首置 MAX(seq)/增量/排除自己 + thread 前缀 + server/channel-members/reactions/search/upload MIME 白名单）；`test/agents-credentials.test.ts` 7 用例（签发 24h TTL 窗口断言 → scoped token 单条索引认证路径 → 轮换 upsert 旧 token 401 → 吊销幂等 → 防自续期/自吊销 403 → 跨 agent 冒用 401 → sha256 落库实查）；`test/preview-integrations.test.ts` 9 用例（路由确定性分支 + SSRF 黑名单路由级矩阵 + 纯函数黑名单/元信息提取离线矩阵 + integrations stub 契约如实断言；真外网抓取不进测试面，502 用 .invalid 保证 NXDOMAIN）；`test/rate-limit.test.ts` 7 用例（离线假 req/reply 直测 hook：NODE_ENV=test 跳过/health 恒放行/query 不分桶/(ip,method,path) 三维隔离/auth 20 / sensitive 5 桶上限/假时钟窗口重置）；`test/reminder-scheduler.test.ts` 8 用例（离线直测 tick：假 app 真库认领（SKIP LOCKED 真事务）+ daemonClients 注入假 ws 收帧断言——无 daemon 跳过/一次性 fire+WS 定向投递+不重排/every:1m 消漂移锚定原 fire_at+60s/P1.23 认领门控（离线 owner 保持 scheduled）/duty off/paused 三类不认领/patrol 沉默累计达上限自动暂停+auto_paused 事件+patrol_paused 通知/patrol 有产出清零+outcome 事件）；`test/ws-validate.test.ts` 12 用例（离线 schema 逐 union 成员对齐：daemon 12 type + browser 5 type 最小合法帧/未知 type/非 JSON/错型字段/枚举越界/passthrough 前向兼容/方向隔离）。② **WS 入站 zod 运行时校验**——`src/ws/validate.ts`（新）：两 union 逐字段对齐 shared（required 照抄），`.passthrough()` 前向兼容；ready 字段全 optional（probe-p127 最小握手形态 + handler 全 if-guard）；opaque 载荷（obs frame/frames）只验「是对象」；`parseWsInbound` 非 JSON/未知 type/错型字段整帧丢弃 + 10s 限频 warn（防劣质客户端刷日志），handler 两处 `JSON.parse(raw) as X` 全部改走校验（校验过帧进 switch 时字段类型得到保证，纵深防御）；`ws.test.ts` 相应用例改断言「合法帧中继到浏览器 + 畸形/未知帧丢弃但不断连」。③ **测试实锤三处真 bug 当场修复**：ⅰ `rate-limit.ts optsFor` sensitive 桶正则的裸 `change-password` 分支要求 `/api/` 后紧跟、对实际路由 `/api/profile/change-password` 永不匹配——改密高危桶此前从未生效，正则改 `profile\/(deactivate|change-password)|auth\/deactivate`；ⅱ `preview.ts isBlockedHost`——`new URL("http://[::1]/").hostname` 带方括号返回 `"[::1]"`，精确比较 `"::1"` 永不命中，IPv6 回环绕过 SSRF 黑名单（测试 502 而非 400 实锤）——剥方括号后再比对（IPv4-mapped IPv6 等 DNS/IP 层绕过归 P1.29 的最终 IP 段校验）；ⅲ `messages.ts` GET /、POST /send、GET /history 三处 legacy 降级频道名解析 `scope = tenant.explicit ? tenant.serverId : undefined` 传 undefined → `WHERE name=$1 LIMIT 1` 无 ORDER BY 跨 server 任取同名频道（命中取决于堆物理布局，测试期大量建删频道搅动布局后稳定复现 tenant.test 157 跨社区串名假象）——改 `tenant.serverId ?? undefined` 圈定默认 server（对齐 tenant.ts「降级=默认 server」文档语义与 tenant.test 断言；RBAC 降级豁免 accessOptsOf 另轴不动；显式租户路径零变化）。④ **测试基建顺手补**：`helpers.ts api()` 补上已声明但从未实现的 `token` 选项（Bearer 头，此前无人传所以零暴露）；已知坑复踩两处——`.env` DATABASE_URL 密码与回退值不一致须显式注入；vitest 假时钟 `advanceTimersByTimeAsync` 对含真实 IO 的 async tick 回调不可靠（tick 未执行静默假败 4 用例），scheduler 测试改真实 100ms 间隔 + 轮询到终态（fire_count≥1/事件行出现），到期判定本就锚定 PG 侧时钟，假时钟要求以「DB 侧插入过去/未来 fire_at」实现。⑤ 验证：server tsc 零错误 + shared/daemon tsc + web vue-tsc 零错误 + **全量 48/48 文件 486 用例绿 ×2**（agent 侧消息面/凭据面/preview 首次有回归网）。**实锤留痕**：① postgres.js jsonb 列返回原始 JSON 字符串非解析对象（测试断言须 `typeof detail === "string" ? JSON.parse…` 防御）；② messages.seq/attachments.size_bytes 等 BIGINT → JSON string（Number() 断言）；③ URL query 裸 `#` 被 fragment 截断（须 encodeURIComponent）；④ `/api/messages/history` 对不带 `#` 的参数按 UUID 直取、带 `#` 走名字解析——两形态并存的历史契约（bare-name 报 22P02 uuid cast，已按 `#name` 修正用例）；⑤ helpers `cleanupTestData` 的 reminders 清理按 user_id 删、而 reminders.owner_id 是 agent id——通用清理覆盖不到，新测试文件均按 patrol.test.ts 模式显式清 reminder_events → reminders。**取舍立此存照**：preview 真·外网抓取路径无 fixture（本机 127.0.0.1/内网段全被黑名单挡、本地 fixture 无可用主机名），确定性分支 + 纯函数覆盖为主，fetch 幸福路径与 redirect 复查归 P1.29 一并处理；integrations 用 HTTP 200+{error} 的 stub 反模式（§2.1 中低）按现状如实断言，收敛 4xx 时（P2 #1）同步更新断言；scheduler 的「假时钟」以 DB 侧 fire_at 控制实现（PG 时钟才是 scheduler 真正服从的时钟，JS 侧唯一时间输入 new Date() 仅参与消漂移重排，断言锚定重排差值而非墙钟）；orgs invites（零覆盖清单 ⑥）不在本批 | `test/agents-messages.test.ts`（新）+ `test/agents-credentials.test.ts`（新）+ `test/preview-integrations.test.ts`（新）+ `test/rate-limit.test.ts`（新）+ `test/reminder-scheduler.test.ts`（新）+ `test/ws-validate.test.ts`（新）+ `src/ws/validate.ts`（新）+ `src/ws/handler.ts` + `src/routes/preview.ts`（导出纯函数 + 剥方括号）+ `src/routes/messages.ts`（legacy 解析 scope）+ `src/lib/rate-limit.ts`（optsFor 正则）+ `test/helpers.ts`（token 选项）+ `test/ws.test.ts` + `package.json`/`pnpm-lock.yaml`（zod ^3.25.76） | 测试盲区 ①~⑤ 全补齐 + WS 纵深防御；顺带消灭 3 个测试网外真缺陷（限流高危桶失效 / IPv6 回环 SSRF 绕过 / legacy 跨社区串名） |
