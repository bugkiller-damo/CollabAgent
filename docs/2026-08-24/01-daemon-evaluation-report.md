...
| 严重度 | 文件:行号 | 问题 | 后果 |
|---|---|---|---|
| 高 | `agent-runtime-dispatch.ts:173` | `createDispatch` 函数长达 840 行，`doDispatch` 同时处理 PTY/headless、MCP 配置、启动超时、reply guard、进度条、thread context | 单函数职责过重，无单元测试，修改极易引入回归 |
| 高 | `agent-runtime.ts:194` | `createAgentRuntime` 函数 625 行，内含 `installStuckDetector` 等大型内联闭包 | 状态机/定时器/回收器/清理链全部挤在一个工厂函数，阅读与单测困难 |
| 高 | `daemon-core.ts:379` | `handleMessage` 巨型 switch，`agent:deliver` 和 `terminal:watch` 单 case 超过 100 行 | WS 消息处理核心无单元测试 |
...
| 中 | `agent-runtime-dispatch.ts:370-448` | PTY 分支与 headless 分支在 `doDispatch` 内并列，各自重复 `mintAgentCredential`、写 token 文件、写系统提示、建 workspace | 两条路径未抽取公共启动准备函数 |
...
- **P0**：为核心编排器补单元测试：拆分 `createAgentRuntime` / `createDispatch` 的依赖注入，针对 `runAgent`、dispatch queue 集成、WS 消息路由、成本熔断、reply guard 等写独立测试。
...
9. **拆分巨型函数/模块**：`createDispatch` → 多个子函数；`handleMessage` → `handlers/*`；`cli.ts` → 按域拆分。
...
| `agent-runtime.ts` | **否** | **无** | 核心编排器，仅靠集成测试间接覆盖 |
| `agent-runtime-dispatch.ts` | **否** | **无** | prompt 函数有测，分发逻辑无测 |
| `agent-runtime-spawn.ts` | **否** | **无** | PTY spawn 冻结代码 |
| `daemon-core.ts` | **否** | **无** | WS 连接/消息处理核心无测 |
| `cli.ts` | **否** | **无** | 整个 CLI 无测 |
