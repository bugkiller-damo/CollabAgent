...
### T4【🟡 中】结构化观察帧即产品（对标 Buzz observer，承接 B1）☐ Step 7 实施中 2026-08-21
- **落地中**：共享 `summarizeProgress`；频道内 ⏳ 进度条（节流 PUT，结束删/改写）；顶栏 `agent:progress`；事件流中文活动卡。设计：`docs/2026-08-21/02-t4-observation-product-design.md`。
...
- **验收**：非技术用户能从观察面板/频道顶栏读懂 agent 进度，无需看终端。
...
| L4 | agent 引导层加固 | ~~系统提示加「网络命令必须带 `--max-time`」~~ ✅ 2026-09-20 已落地（A3 重写版 system-prompt「真实限制」节） | 已完成 |
| L5 | 已知边界调优 | ~~超长生成 >300s 被不活跃超时误杀~~（A7.6 真机证伪：长工具每 ~30s 发 `tool_progress` 心跳，不触发沉默超时）；~~persistent 路径闲置回收疑似未覆盖~~（评估 P0.2 已修）；grok 中转下 WebFetch 不可用仍在 | 按需调参，文档已记 |
...
| T4 | 观察帧升级为产品级活动面板 + 频道进度 | 🟡 中 | 中 | B1 ✅ | ✅ 2026-08-21 Step 7 |
...
| L4 | 网络命令 `--max-time` 引导加固 | 🟢 低 | 随手 | — | ✅ 2026-09-20（A3 prompt 已含：`system-prompt.ts`「网络命令一律带 `--max-time`」） |
| L5 | 已知边界调优 | 🟢 低 | 小 | — | ✅ 大半 2026-09-20：闲置回收已由评估 P0.2 修复；>300s 误杀经 A7.6 真机证伪（长工具有 30s `tool_progress` 心跳）；仅 grok 中转 WebFetch 限制仍在 |
