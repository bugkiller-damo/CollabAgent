...
> **评估报告 P1.16 移除 `SLOCK_DISPATCH_QUEUE=0` 回退路径已落地**（2026-08-26，`b19b269`）。
> 至此评估报告 P1 八项（P1.9–P1.16）全部完成。
> 下一焦点：P2 批（均未启动；P2.17 PTY 代码删除计划按原定 2026-09 底评估）。
...
| 评估 P1.14 | 2026-08-26 | `0ec3991` | 统一错误模型：`DispatchError(code)` + `retriable` 由 code 推导（agent-unknown/agent-stopped/session-lost 永久失败，inflight-timeout/credential-mint-failed 可重试）；队列对非可重试错误首次失败即死信不空转退避，未分类 Error 保持旧「一律重试」；派发链 5 处抛点迁移；`test/errors.test.ts` +6、队列 +3；typecheck + 38 文件 359 用例全绿 |
| 评估 P1.15 | 2026-08-26 | `3d92ddc` | token 脱敏 + `.slock` 0700：新建 `redact.ts`（`redactSecrets`/`redactDeep`，`sk_(agent\|machine)_[a-z0-9]{4,}` → 前缀 + `***`）与 `private-dir.ts`（`mkdirPrivateSync`）；观察帧源头脱敏（truncate 先脱敏再截断 + push 深脱敏含 toolInput）；terminal-log 落盘前脱敏；10 处 `.slock` mkdir 统一 0700；`test/redact.test.ts` 8 例 + `test/terminal-log.test.ts` 3 例 + 观察帧/token-file 补 4 例；typecheck + 40 文件 374 用例全绿 |
| 评估 P1.16 | 2026-08-26 | `b19b269` | 移除 `SLOCK_DISPATCH_QUEUE=0` 回退路径：派发队列成为唯一入口（`createDispatchQueue` 不再判 env，`dispatchToAgent` 直连队列）；config.ts 删 `dispatchQueue` 字段。至此评估 P1 批（P1.9–P1.16）全清 |
