...
| 评估 P1.15 | 2026-08-26 | （待提交） | token 脱敏 + `.slock` 0700：新建 `redact.ts`（`redactSecrets`/`redactDeep`，`sk_(agent\|machine)_[a-z0-9]{4,}` → 前缀 + `***`）与 `private-dir.ts`（`mkdirPrivateSync`）；观察帧源头脱敏（truncate 先脱敏再截断 + push 深脱敏含 toolInput）；terminal-log 落盘前脱敏；10 处 `.slock` mkdir 统一 0700；`test/redact.test.ts` 8 例 + `test/terminal-log.test.ts` 3 例 + 观察帧/token-file 补 4 例；typecheck + 40 文件 374 用例全绿 |
