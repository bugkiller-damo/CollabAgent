...
本次实测（批次 A 落地后复测）：Windows 11 x64 下 CPython **3.10.21 / 3.12.14 / 3.13.15** 各跑 `pytest tests -q` → **134 passed / 0 skip**；`ruff check slock_runtime tests` clean；`mypy slock_runtime` clean；daemon 侧 `vitest run test/runtime-entrypoint-probe.test.ts test/sarp-python-worker.test.ts` → **22/22 passed**。批次 A 前基线为 121 passed。远端首跑 `python-sdk.yml` run `35594027408` 已完成：12 个 job 中 7 个通过、5 个失败——macOS smoke、CentOS Stream 9、Ubuntu/Windows Python 3.13、Ubuntu daemon bridge、quality 通过；4 个 Python 3.10 leg 因测试 harness 的 `StringIO.seek(0)` 移动共享写游标造成撕帧失败，Windows daemon bridge 因 `session.stop()` 未等待 worker 退出导致 `runtime-state.sqlite` 删除 `EBUSY`；Windows 3.10 另有一个零帧超时暂归类为待复跑 flake。两处确定性修复已本地验证，待第二轮远端 CI。未重新运行 server/web 测试，不虚构其结果。
...
| Python CI | 部分完成 | `.github/workflows/python-sdk.yml` 已配置并首跑（run `35594027408`：ubuntu-22.04/24.04 + windows-2022 × 3.10/3.13 全量、ubuntu-24.04 3.10 minimum-dependencies job、macOS 3.12 probe+单回合冒烟、ubuntu-24.04/windows-2022 daemon-bridge 集成 leg、centos-stream-9 容器 job、单 ubuntu-24.04 quality leg）；daemon Python 依赖测试经 `SLOCK_TEST_PYTHON`/`python3`/`python` 候选探测选择解释器 | 首跑 7/12 绿；4 个 3.10 leg 的测试 harness 撕帧与 Windows daemon bridge 的 SQLite `EBUSY` 已修，待第二轮远端验证 |
...
> 实施进度（2026-09-21 批次 A）：P0.1 发布物/API 分层已落地且 clean-venv 安装本地验证通过，**PyPI 未发布**（trusted-publisher 映射与 `pypi` environment 属外部配置，尚未完成）；P0.2 已完成（SDK 内置 probe + 双模板）；P0.5 workflow 已配置并完成首跑（run `35594027408`：7/12 绿，两类测试层问题已修，**待第二轮远端 CI**）；P0.6 已完成（quickstart + README）；P0.3/P0.4/P0.7 未动，Developer Preview 门槛**未全部达成**。
...
- 实施记录（2026-09-21，本批已落地）：
  ...
  - 未做/未验：修复后的第二轮远端 CI 尚未运行；PyPI 未发布且 trusted-publisher/environment 外部配置未完成；Ubuntu/Windows Server 的 3.10 leg 与 Windows daemon bridge 需第二轮 CI 确认；P0.3/P0.4/P0.7 不在本批。
...
| 验证面 | 当前覆盖 | 需新增 |
|--------|----------|--------|
| Python unit | `bridges/python/tests` 11 文件 134 用例（Windows 本地 3.10.21/3.12.14/3.13.15 各 134/134 无 skip；远端首跑中 Ubuntu/Windows 3.13 与 CentOS 3.11 通过，4 个 3.10 leg 因测试 harness `StringIO.seek(0)` 撕帧失败，已修为 `getvalue()` 快照并本地复验） | 第二轮远端 CI 确认 3.10 legs；发布的 wheel 在 clean venv 安装后跑 smoke（release workflow 已配置） |
...
| daemon bridge/session | `persistent-jsonl-worker.test.ts`、`jsonl-bridge-runtime.test.ts`、`sarp-bridge-integration.test.ts`（mjs fixture 真进程）、`sarp-python-worker.test.ts`（真 SDK，候选探测解释器）、`sarp-langgraph-e2e.test.ts`（真 LangGraph+sqlite，跨平台 Python 候选探测）、`sarp-template-manifest-e2e.test.ts`（manifest→probe→driver→模板 worker，`SLOCK_TEST_PYTHON` 门控） | 远端首跑中 ubuntu-24.04 leg 与 centos-stream-9 通过，windows-2022 leg 因 `stop()` 未等待 worker 退出而 `EBUSY`；已改为可 await 的 stop Promise 并本地复验，待第二轮 CI；`slock runtime check` CLI 测试 |
...
| 多 OS smoke | Windows 11 x64 本地已验证（3.10/3.12/3.13 全链路）；远端首跑中 macOS 3.12 smoke、CentOS Stream 9、Ubuntu/Windows 3.13 与 Ubuntu daemon bridge 通过；Ubuntu/Windows 3.10 与 Windows daemon bridge 的已识别测试问题待第二轮 CI 确认 | 第二轮远端 CI 全绿后把 Ubuntu/Windows Server 升级为已验证 |
