...
export async function agentRoutes(app: FastifyInstance) {
  ...
  app.post("/", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const result = await app.pg.query(
      ...
      [userId, serverId as string, name as string, (displayName || name) as string, description || "", sql.json({ runtime: (runtime as string) || "claude", model: (model as string) || "sonnet" })]
    );
    const agent = result.rows[0] as any;
    const rp = (typeof agent.runtime_profile === "string" ? JSON.parse(agent.runtime_profile) : agent.runtime_profile) || {};
    ...
    // 403 "not your agent"。
    sendToDaemon(String(agent.user_id), { type: "agent:start", agentId: agent.id, config: { name: agent.name, displayName: agent.display_name, description: agent.description, runtime: rp.runtime || "claude", model: rp.model || "sonnet" } });
    return { agent };
  });
  ...
  app.post("/:agentId/channels/:channelName/join", { preHandler: [app.authenticate, requireOwnAgent] }, async (req, reply) => {
    const { agentId, channelName } = req.params as Record<string, string>;
    const name = channelName.replace(/^#/, "").split(":")[0];
    const ch = await app.pg.query<{ id: string; type: string }>("SELECT id, type FROM channels WHERE name = $1", [name]);
  ...
  app.post("/:agentId/channels/:channelName/leave", { preHandler: [app.authenticate, requireOwnAgent] }, async (req, reply) => {
    const { agentId, channelName } = req.params as Record<string, string>;
    const name = channelName.replace(/^#/, "").split(":")[0];
