...
export async function metricsRoutes(app: FastifyInstance) {
  ...
  app.get("/metrics/history", { preHandler: [app.authenticate] }, async (req) => {
    ...
    const r = await app.pg.query(
      `SELECT sampled_at, messages_sent, dm_sent, reminders_fired, errors, logins, rss_mb, heap_used_mb, heap_total_mb, daemon_count, agent_total, agent_online FROM metrics_samples WHERE sampled_at > now() - ($1 || ' hours')::interval ORDER BY sampled_at ASC`,
      [hours],
    );
