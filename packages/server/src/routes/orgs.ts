...
export async function orgRoutes(app: FastifyInstance) {
  ...
  app.get("/orgs", { preHandler: [app.authenticate] }, async (req: any) => {
    try { await getOrCreatePersonalOrg(app, req.user.sub, req.user.handle); } catch { /* ignore */ }
  ...
  app.get("/orgs/:serverId/members", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    ...
    const me = await app.pg.query("SELECT 1 FROM server_members WHERE server_id = $1 AND user_id::text = $2", [serverId, req.user.sub]);
  ...
  app.post("/orgs/:serverId/members", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    ...
    if (!(await isOrgOwner(app, serverId, req.user.sub))) return reply.status(403).send({ error: "only org owner can invite" });
    const u = await app.pg.query<{ id: string }>("SELECT id FROM users WHERE handle = $1", [String(handle).replace(/^@/, "")]);
    if (u.rows.length === 0) return reply.status(404).send({ error: "user not found" });
    await app.pg.query("INSERT INTO server_members (server_id, user_id, role) VALUES ($1, $2, 'member') ON CONFLICT DO NOTHING", [serverId, u.rows[0]!.id]);
    return { ok: true };
  });

  app.delete("/orgs/:serverId/members/:userId", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    const { serverId, userId } = req.params;
    if (!(await isOrgOwner(app, serverId, req.user.sub))) return reply.status(403).send({ error: "only org owner can remove" });
    await app.pg.query("DELETE FROM server_members WHERE server_id = $1 AND user_id = $2 AND role <> 'owner'", [serverId, userId]);
    return { ok: true };
  });
  ...
  app.patch("/orgs/:serverId/members/:userId", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    ...
    if (!(await isOrgOwner(app, serverId, req.user.sub))) return reply.status(403).send({ error: "only org owner can change roles" });
    await app.pg.query("UPDATE server_members SET role = $3 WHERE server_id = $1 AND user_id = $2 AND role <> 'owner'", [serverId, userId, role]);
    return { ok: true };
  });
  ...
  app.get("/orgs/:serverId/invites", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    const { serverId } = req.params;
    if (!(await isOrgOwner(app, serverId, req.user.sub))) return reply.status(403).send({ error: "only org owner can manage invites" });
    const r = await app.pg.query(`SELECT token, role, max_uses, uses, expires_at, revoked_at, created_at FROM invites WHERE server_id = $1 ORDER BY created_at DESC`, [serverId]);
  ...
  app.post("/orgs/:serverId/invites", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    ...
    await app.pg.query(`INSERT INTO invites (token, server_id, created_by, role, max_uses, expires_at) VALUES ($1, $2, $3, 'member', $4, $5)`, [token, serverId, req.user.sub, maxUses ? Number(maxUses) : null, expiresAt]);
    return { token };
  });
  ...
  app.delete("/orgs/:serverId/invites/:token", { preHandler: [app.authenticate] }, async (req: any, reply: any) => {
    ...
    if (!(await isOrgOwner(app, serverId, req.user.sub))) return reply.status(403).send({ error: "only org owner can revoke" });
  ...
  app.get("/invites/:token", async (req: any, reply: any) => {
    ...
    const r = await app.pg.query(
      `SELECT i.server_id, i.role, i.max_uses, i.uses, i.expires_at, i.revoked_at, s.name AS server_name
         FROM invites i JOIN servers s ON s.id = i.server_id WHERE i.token = $1`, [token]
    );
    if (r.rows.length === 0) return reply.status(404).send({ error: "邀请链接无效" });
    const inv = r.rows[0] as { revoked_at: string | null; expires_at: string | null; max_uses: number | null; uses: number; server_name: string };
  ...
  app.get("/server/info", { preHandler: [app.authenticate] }, async (req: any) => {
    ...
    return { serverId, serverName: serverResult.rows[0]?.name, channels: channels.rows, agents: [], humans: humans.rows };
  });
}
