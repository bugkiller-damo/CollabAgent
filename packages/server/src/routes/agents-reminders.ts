...
export async function agentReminderRoutes(app: FastifyInstance) {
  app.post("/:agentId/reminders", { preHandler: [app.authenticate, requireOwnAgent] }, async (req, reply) => {
    ...
    const r = await app.pg.query(
      "INSERT INTO reminders (owner_id, title, fire_at, repeat_rule, channel_ref, status) VALUES ($1, $2, $3, $4, $5, 'scheduled') RETURNING *",
    ...
    async (req, reply) => {
      ...
      return {
        ...
        events: [
          ...
          { event: dto.status, at: r.rows[0].updated_at },
        ],
      };
    },
  );
