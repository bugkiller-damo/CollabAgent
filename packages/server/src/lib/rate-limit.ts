...
interface RateLimitOpts { windowMs: number; max: number; message?: string }
...
class MemoryBackend {
  ...
  async check(key: string, opts: RateLimitOpts) {
    ...
    if (!record || record.resetAt < now) { this.store.set(key, { count: 1, resetAt: now + opts.windowMs }); return { allowed: true, remaining: opts.max - 1 }; }
    record.count++;
    return record.count > opts.max ? { allowed: false, remaining: 0 } : { allowed: true, remaining: opts.max - record.count };
  }
...
class RedisBackend {
  ...
  async check(key: string, opts: RateLimitOpts) {
    try { const r = await this.r.multi().incr(key).pexpire(key, opts.windowMs, "NX").exec(); return { allowed: r[0][1] <= opts.max, remaining: Math.max(0, opts.max - r[0][1]) }; }
