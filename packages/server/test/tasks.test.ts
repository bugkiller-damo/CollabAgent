...
describe("tasks: dispatch 派发同步看板（P1）", () => {
  it("派发建卡片(in_progress)→回报(in_review)；再派发→撤回(closed)", async () => {
    ...
    const mk = async (name: string) => {
      const comp = await ensureTestComputer(a);
  ...
  it("agent 侧 claim 同样走条件更新（agent A 认领后 agent B 冲突）", async () => {
    ...
    const mkAgent = async (name: string) => {
      ...
      const r = await api("/api/agents", {
        ...
        cookie: owner.cookie,
        csrf: owner.csrf,
        body: { name, displayName: name, serverId: comp.serverId },
      });
      expect(r.status).toBe(200);
...
describe("P1.33: unclaim/update-status 归属校验", () => {
  ...
  it("认领人是 agent：其 owner（非频道管理）可动，其他成员 403", async () => {
    ...
    const bComp = await ensureTestComputer(b);
    const ag = (
      await api("/api/agents", {
        method: "POST",
        cookie: b.cookie,
        body: { name: "tko_" + uniqHandle(), displayName: "tko", serverId: bComp.serverId },
      })
    ).data.agent;
    expect(
