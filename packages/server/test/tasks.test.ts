...
describe("tasks: 消息转任务（from-message）", () => {
  it("既有消息可转为任务并出现在看板，重复转换 409，消息不存在 404", async () => {
    ...
    // 转为任务
    const conv = await api("/api/tasks/from-message", { method: "POST", cookie: a.cookie, body: { message_id: messageId } });
    ...
    // 重复转换 → 409
    const dup = await api("/api/tasks/from-message", { method: "POST", cookie: a.cookie, body: { message_id: messageId } });
    expect(dup.status).toBe(409);

    // 消息不存在 → 404
    const missing = await api("/api/tasks/from-message", { method: "POST", cookie: a.cookie, body: { message_id: crypto.randomUUID() } });
...
describe("tasks: dispatch 派发同步看板（P1）", () => {
  it("派发建卡片(in_progress)→回报(in_review)；再派发→撤回(closed)", async () => {
    ...
    const chName = uniqHandle();
    await api("/api/channels", { method: "POST", cookie: a.cookie, body: { name: chName, description: "dispatch sync" } });

    // 建经理 + worker 两个 agent 并拉进频道
    const mk = async (name: string) => {
      const r = await api("/api/agents", { method: "POST", cookie: a.cookie, body: { name, runtime: "claude", model: "sonnet" } });
    ...
    for (const ag of [mgr, wkr]) {
      const inv = await api(`/api/channels/${channelId}/invite`, { method: "POST", cookie: a.cookie, body: { handle: ag.name } });
      expect(inv.status).toBe(200);
    }
    // 指定经理
    const pm = await api(`/api/channels/${channelId}/members/${mgr.id}`, { method: "PATCH", cookie: a.cookie, body: { is_manager: true } });
    expect(pm.status).toBe(200);

    // 派发 → 看板出现 in_progress 卡片，assignee 是 worker
    const d1 = await api(`/internal/agent/${mgr.id}/dispatch`, { method: "POST", cookie: a.cookie, body: { channel: "#" + chName, toAgent: wkr.name, text: "做个登录页" } });
    ...
    // 回报 → in_review
    const rep = await api(`/internal/agent/${wkr.id}/dispatch/${d1.data.dispatch.id}/report`, { method: "POST", cookie: a.cookie, body: { reportText: "已完成" } });
    ...
    const cancel = await api(`/internal/agent/${mgr.id}/dispatch/${d2.data.dispatch.id}/cancel`, { method: "POST", cookie: a.cookie, body: { reason: "不需要了" } });
