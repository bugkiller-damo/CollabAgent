...
describe("direct messages: 三向收发 + 成员隔离", () => {
  it("两个用户互发 DM 命中同一频道，双方可读", async () => {
    ...
    const readByA = await api(`/api/messages?channel=${encodeURIComponent("dm:@" + b.handle)}`, { cookie: a.cookie });
  ...
  it("非成员无法读取他人 DM 频道（403）", async () => {
    ...
    const peek = await api(`/api/messages?channel=${encodeURIComponent("dm:" + channelId)}`, { cookie: c.cookie });
