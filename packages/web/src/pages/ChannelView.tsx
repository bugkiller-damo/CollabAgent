...
import { type ListItem, VirtualMessageList } from "../components/chat/VirtualMessageList";
...
export function ChannelView() {
  ...
  const [pending, setPending] = useState<
    { tempId: string; content: string; status: "sending" | "failed" | "queued" }[]
  >([]);
  ...
  useEffect(() => {
    ...
    apiGet<{ results: { id: string; seq: number }[] }>("/api/messages/search", { q: highlightMsgId.slice(0, 8) })
      .then((r) => {
    ...
    <div className="flex min-h-0 flex-1">
      ...
      >
        ...
        <PageHeader
          ...
          leading={
            (currentChannel as any)?.type === "private" || (currentChannel as any)?.visibility === "private"
              ? lockIcon
              : undefined
          }
        >
          <div className="flex items-center gap-1">
            <IconButton
              ...
              onClick={() => {
                ...
                const working = Object.values(agents).find(
                  (a) => a.status === "working" || (a.status as string) === "thinking",
                );
            ...
            <IconButton label="看板" tooltip="任务看板" onClick={() => navigate("/tasks/" + channelName)}>
              {boardIcon}
            </IconButton>
            <IconButton
              label="成员"
              ...
              className={showMembers ? "text-blue-500" : ""}
            >
              {membersIcon}
            </IconButton>
            {currentChannel && (
              <IconButton label="频道设置" tooltip="频道设置" onClick={() => setShowSettings(true)}>
                {settingsIcon}
              </IconButton>
            )}
          </div>
