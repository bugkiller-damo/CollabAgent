...
async function deactivate() {
  ...
  busy.value = true;
  try {
    await apiClient("/api/profile/deactivate", { method: "POST", body: { password: pwd.value } });
    toast.success("账户已注销");
    authStore.logout();
    router.push("/login");
  } catch (e: any) {
...
<template>
  <div class="w-full space-y-8">
    <PageHeader title="安全与账户" back-to="/settings" />
    ...
    <section>
      ...
      <div v-else class="space-y-2">
        <Card v-for="s in sessions" :key="s.id" padding="sm" class="flex items-center justify-between">
          ...
          <Button v-if="!s.current" @click="revoke(s.id)" variant="ghost" size="sm" class="shrink-0 text-red-500 hover:text-red-600">下线</Button>
        </Card>
      </div>
    </section>
    ...
    <section class="rounded-lg border border-red-300 p-4 dark:border-red-900/50">
      <h3 class="mb-2 font-bold text-red-600 dark:text-red-400">注销账户</h3>
      <p class="mb-3 text-sm text-gray-500">注销后将无法登录，个人信息会被清除（历史消息保留）。此操作不可轻易撤销。</p>
      <div class="max-w-sm space-y-2">
        <Input type="password" :value="pwd" @input="pwd = ($event.target as HTMLInputElement).value" placeholder="输入密码确认" />
        <Input :value="confirmText" @input="confirmText = ($event.target as HTMLInputElement).value" placeholder="输入「注销」二字确认" />
