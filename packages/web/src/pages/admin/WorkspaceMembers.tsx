  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
      <PageHeader
        title="成员管理"
        backTo="/admin"
        breadcrumb={[{ label: "管理后台", to: "/admin" }, { label: "成员管理" }]}
      />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  生成链接发给同事，他们用链接注册后自动加入「{org?.name}」。
                </p>
              </div>
            ...
            {activeInvites.map((inv) => (
              <div key={inv.token} className="flex items-center gap-2 rounded bg-gray-100 p-2 dark:bg-gray-900">
                ...
                <span className="shrink-0 text-xs text-gray-400">
                  已用 {inv.uses}
                  {inv.max_uses != null ? `/${inv.max_uses}` : ""} 次
                  {inv.expires_at && ` · ${new Date(inv.expires_at).toLocaleDateString()} 过期`}
                </span>
                <Button onClick={() => copyInvite(inv.token)} variant="ghost" size="sm" className="shrink-0">
