...
<template>
  <div class="flex h-full flex-col">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto">
      ...
      <div v-else-if="list.length === 0" class="px-4 py-8 text-center text-sm text-gray-500">暂无通知</div>
      <button
        v-for="n in list"
        :key="n.id"
        :class="[
          'flex w-full gap-2 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-200 dark:border-gray-700 dark:hover:bg-gray-700',
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <p v-if="n.body" class="mt-0.5 line-clamp-2 text-xs text-gray-600 dark:text-gray-400">{{ n.body }}</p>
