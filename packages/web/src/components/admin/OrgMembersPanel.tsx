    ...
    <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 space-y-3">
      <div>
        ...
        <p className="text-gray-500 text-xs mt-0.5">
          加入这里的成员能看到你在此空间创建的 Agent（共 {org.agentCount} 个）。新建 Agent 默认进入这里、仅你可见。
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {members.map((m) => (
          <span
            key={m.user_id}
            className="group inline-flex items-center gap-1.5 text-sm bg-gray-200 dark:bg-gray-700 rounded-full pl-2.5 pr-1.5 py-1 text-gray-700 dark:text-gray-200"
          >
            @{m.handle}
