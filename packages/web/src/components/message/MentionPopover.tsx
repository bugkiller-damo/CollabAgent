...
export function MentionPopover({
  ...
  useEffect(() => {
    apiGet<{ users: Member[]; agents: Member[] }>("/api/server/info")
      .then((d) => setMembers([...(d.users || []), ...(d.agents || [])]))
      .catch(() => {});
  }, []);
      ...
      {list.map((m, i) => (
        <button
          ...
          className={
            ...
            (i === idx ? "bg-blue-600 text-white" : "text-gray-300 hover:bg-gray-700")
          }
        >
          <span className="w-6 h-6 rounded bg-gray-600 flex items-center justify-center text-xs shrink-0">
            {m.handle[0]}
          </span>
