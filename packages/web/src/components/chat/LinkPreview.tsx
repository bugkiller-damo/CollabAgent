...
export function LinkPreview({ url }: { url: string }) {
  ...
  return (
    ...
    >
      {data.image && <img src={data.image} alt="" loading="lazy" className="w-20 h-20 object-cover shrink-0" />}
      <div className="min-w-0 py-2 pr-2">
        {data.siteName && <div className="text-[11px] text-gray-400 truncate">{data.siteName}</div>}
        {data.title && (
          <div className="text-sm text-gray-800 dark:text-gray-200 font-medium truncate">{data.title}</div>
        )}
