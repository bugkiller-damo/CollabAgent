...
export function ChannelMembersPanel({ channelId, onClose }: { channelId: string; onClose: () => void }) {
  ...
  const handleManager = async (m: Member, is_manager: boolean) => {
    try {
      ...
      toast.error(
        err?.message === "channel already has a manager"
          ? "该频道已有经理，请先取消原经理"
          : err?.message || "设置失败",
      );
    }
  ...
  const renderMember = (m: Member) => {
    ...
    return (
      <div
        ...
        className="group flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700"
      >
        <div
          className={
        ...
        <div className="flex-1 min-w-0">
          <div className="text-gray-800 dark:text-gray-200 text-sm truncate">
            ...
            {isSelf && <span className="text-gray-400"> （你）</span>}
          </div>
  ...
  return (
    <aside className="w-60 shrink-0 border-l border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex flex-col">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-gray-700 space-y-1">
        ...
        {inviteMsg && (
          <p className={"text-xs " + (inviteMsg === "已邀请" ? "text-green-500" : "text-red-400")}>{inviteMsg}</p>
        )}
      </div>

      <div className="flex-1 overflow-y-auto p-2 space-y-3">
        {loading && <p className="text-gray-400 text-sm text-center py-4">加载中…</p>}
        {!loading && members.length === 0 && <p className="text-gray-400 text-sm text-center py-4">暂无成员</p>}
        {agents.length > 0 && (
          <div>
