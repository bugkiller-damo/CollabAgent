...
export function OnboardingChecklist() {
  ...
  useEffect(() => {
    if (dismissed) return;
    const load = () => {
      apiGet<{ connected: boolean }>("/api/daemon/status").then((d) => setDaemonOn(!!d.connected)).catch(() => setDaemonOn(false));
      apiGet<{ agents: any[] }>("/api/agents").then((d) => setHasAgent((d.agents || []).length > 0)).catch(() => setHasAgent(false));
    };
    load();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-72 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 p-4">
      <div className="flex items-start justify-between mb-2">
        ...
        <button onClick={dismiss} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-sm" title="不再显示">✕</button>
      </div>
      <ul className="space-y-2">
        {steps.map((s) => (
          <li key={s.label} className="flex items-center gap-2 text-sm">
            <span className={s.done ? "text-green-500" : "text-gray-400"}>{s.done ? "✓" : "○"}</span>
            <span className={s.done ? "text-gray-400 line-through" : "text-gray-700 dark:text-gray-200"}>{s.label}</span>
          </li>
        ))}
      </ul>
      <Link to={next.to} className="mt-3 block text-center bg-blue-600 text-white text-sm py-2 rounded hover:bg-blue-500">
        {next.cta}
      </Link>
    </div>
  );
