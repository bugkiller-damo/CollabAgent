...
export function generateSystemPrompt(agent: AgentIdentity, channelName?: string, dispatchContext?: DispatchContext | null): string {
  ...
  lines.push(
    ...
    `- **优先用 \`send_message\`/\`list_tasks\`/\`create_tasks\`/\`claim_tasks\`/\`update_task_status\`/\`unclaim_task\`/\`schedule_reminder\`/\`dispatch_task\`/\`list_dispatches\`/\`report_task\`/\`cancel_dispatch\` 这些 MCP 工具**（如果可用，会出现在你的工具列表里）——它们是结构化调用，比敲命令行更可靠。`,
    `- 其它这些工具还没覆盖的操作（读历史/查新消息/搜索/加表情/看服务器/看频道成员/资料/上传附件/列出或取消提醒等），继续用本机的 \`slock\` CLI（Bash 工具运行）。`,
    ``,
    `## 可用命令（当前已实现）`,
    ...
    `- 提醒：新建优先用 \`schedule_reminder\` 工具；没有该工具时退回 \`slock reminder schedule --title "看看PR合了没" --in 2h --channel "#${ch}"\`；列出/取消目前仍用 \`slock reminder list\` / \`reminder cancel --id <id>\`。`,
    `- 附件：\`slock attachment upload --path <本地文件>\` 上传得到 attachmentId，再用 \`send_message\` 或 \`slock message send --target "#${ch}" --attachment-id <id>\` 随消息发出；\`attachment view --id <id> --output <路径>\` 下载。`,
    ``,
    `## 任务协作`,
