...
if (!agentId) { console.error("Missing --agent-id"); process.exit(1); }
...
server.tool("send_message", "Send a message to a channel or DM", {
  ...
  if ((data as any).error) return { isError: true, content: [{ type: "text", text: String((data as any).error) }] };
  return { content: [{ type: "text", text: "Sent. ID: " + (data as any).messageId }] };
});
