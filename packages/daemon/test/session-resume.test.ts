...
describe("session resume (agent-runtime-spawn.ts, fake PTY)", () => {
  ...
  it(
    "when enabled and the PTY survives the grace window: injects --resume <id> and skips the restart summary",
    async () => {
      process.env.SLOCK_SESSION_RESUME = "1";
      runStore.saveRuntimeState({
        agentId: AGENT_ID, agentName: AGENT_NAME, status: "idle", lastTransitionAt: Date.now(),
        totalRuns: 1, currentRunId: null, lastSessionId: "good-session-id", lastSessionUpdatedAt: Date.now(),
      });
      // 有历史 run 记录：如果没跳过 restart-summary 注入，bootstrap 文本里
      // 会带上"最近运行摘要"这类文字——用它来验证 didResume 真的抑制了这段注入。
      runStore.insertAgentRun({
        runId: "prior-run", agentId: AGENT_ID, agentName: AGENT_NAME, status: "exited",
        exitCode: 0, startedAt: Date.now() - 60_000, endedAt: Date.now() - 50_000,
        messagesProcessed: 3, lastTurnDuration: 1200,
      });
  ...
  it(
    "when enabled and the PTY exits quickly (simulated failed resume): clears the saved session id and retries once without --resume",
    async () => {
      process.env.SLOCK_SESSION_RESUME = "1";
      runStore.saveRuntimeState({
        agentId: AGENT_ID, agentName: AGENT_NAME, status: "idle", lastTransitionAt: Date.now(),
        totalRuns: 1, currentRunId: null, lastSessionId: "poisoned-session-id", lastSessionUpdatedAt: Date.now(),
      });
  ...
  it(
    "when the PTY dies AFTER the grace window (but before bootstrap is written): still recovers by redelivering via a fresh spawn, instead of silently writing into a dead run",
    async () => {
