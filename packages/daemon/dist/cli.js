...
// Auth
// ---------------------------------------------------------------------------
function registerWhoami(parent) {
    parent
        .command("whoami")
...
// Server
// ---------------------------------------------------------------------------
function registerServerInfo(parent) {
    parent
        .command("info")
        ...
        if (!res.ok)
            fail("INFO_FAILED", res.error ?? `HTTP ${res.status}`);
...
// ---------------------------------------------------------------------------
function registerChannelMembers(parent) {
    parent
        .command("members")
        ...
        if (!res.ok)
            fail("MEMBERS_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerChannelJoin(parent) {
    parent
        .command("join")
        ...
        if (!res.ok)
            fail("JOIN_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(`Joined ${opts.target}\n`);
    });
}
function registerChannelLeave(parent) {
    parent
        .command("leave")
        ...
        if (!res.ok)
            fail("LEAVE_FAILED", res.error ?? `HTTP ${res.status}`);
...
// Thread
// ---------------------------------------------------------------------------
function registerThreadUnfollow(parent) {
    parent
        .command("unfollow")
        ...
        if (!res.ok)
            fail("UNFOLLOW_FAILED", res.error ?? `HTTP ${res.status}`);
...
// ---------------------------------------------------------------------------
function registerMessageSend(parent) {
    parent
        .command("send")
        ...
        .option("--attachment-id <id>", "Attachment ID to link (repeatable)", (v, prev = []) => prev.concat(v))
        .action(async (opts) => {
        const ctx = loadAgentContext();
        const client = new ApiClient(ctx);
        ...
        if (!opts.sendDraft) {
            ...
            if (!content.trim())
                fail("SEND_NO_CONTENT", "No message content received on stdin.");
        }
        ...
        if (!res.ok)
            fail("SEND_FAILED", res.error ?? `HTTP ${res.status}`);
        const data = res.data;
        if (data.state === "held") {
...
function registerMessageCheck(parent) {
    parent
        .command("check")
        ...
        if (!res.ok)
            fail("CHECK_FAILED", res.error ?? `HTTP ${res.status}`);
        const data = res.data;
...
function registerMessageRead(parent) {
    parent
        .command("read")
...
function registerMessageSearch(parent) {
    parent
        ...
        if (opts.channel)
            params.set("channel", opts.channel);
        if (opts.sender)
            params.set("sender", opts.sender);
        if (opts.limit)
            params.set("limit", opts.limit);
        const res = await client.request("GET", `/internal/agent/${encodeURIComponent(ctx.agentId)}/search?${params.toString()}`);
        if (!res.ok)
            fail("SEARCH_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerMessageReact(parent) {
    parent
        .command("react")
        ...
        if (!res.ok)
            fail("REACT_FAILED", res.error ?? `HTTP ${res.status}`);
...
// ---------------------------------------------------------------------------
function registerAttachmentUpload(parent) {
    parent
        .command("upload")
        ...
        if (!fs.existsSync(filePath))
            fail("UPLOAD_FILE_NOT_FOUND", `File not found: ${filePath}`);
        ...
        if (!res.ok)
            fail("UPLOAD_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerAttachmentView(parent) {
    parent
        .command("view")
        ...
        if (!res.ok)
            fail("VIEW_FAILED", `HTTP ${res.status}`);
        const fs = await import("node:fs");
...
// ---------------------------------------------------------------------------
function registerTaskList(parent) {
    parent
        .command("list")
        ...
        if (!res.ok)
            fail("TASK_LIST_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerTaskCreate(parent) {
    parent
        .command("create")
        .description("Create one or more tasks in a channel")
        .requiredOption("--channel <target>", "Channel target")
        .argument("[titles...]", "Task titles")
        .action(async (titles, opts) => {
        if (!titles.length)
            fail("TASK_NO_TITLES", "At least one task title is required");
        const ctx = loadAgentContext();
        const client = new ApiClient(ctx);
        ...
        if (!res.ok)
            fail("TASK_CREATE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerTaskClaim(parent) {
    parent
        .command("claim")
        ...
        if (opts.number)
            body.task_numbers = [Number(opts.number)];
        if (opts.messageId)
            body.message_ids = [opts.messageId];
        const res = await client.request("POST", `/internal/agent/${encodeURIComponent(ctx.agentId)}/tasks/claim`, body);
        if (!res.ok)
            fail("TASK_CLAIM_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerTaskUnclaim(parent) {
    parent
        .command("unclaim")
        ...
        if (!res.ok)
            fail("TASK_UNCLAIM_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write("Task unclaimed\n");
    });
}
function registerTaskUpdate(parent) {
    parent
        .command("update")
        ...
        if (!res.ok)
            fail("TASK_UPDATE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
// ---------------------------------------------------------------------------
// Profile: show, update
// ---------------------------------------------------------------------------
function registerProfileShow(parent) {
    parent
        .command("show")
        ...
        if (!res.ok)
            fail("PROFILE_SHOW_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerProfileUpdate(parent) {
    parent
        .command("update")
        ...
        if (!opts.displayName && !opts.description)
            fail("PROFILE_NO_CHANGES", "At least one field is required");
        const ctx = loadAgentContext();
        const client = new ApiClient(ctx);
        const body = {};
        if (opts.displayName)
        ...
        if (!res.ok)
            fail("PROFILE_UPDATE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
// ---------------------------------------------------------------------------
...
function registerIntegrationList(parent) {
    parent
        ...
        if (!res.ok)
            fail("INTEGRATION_LIST_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerIntegrationLogin(parent) {
    parent
        .command("login")
        ...
        if (!res.ok)
            fail("INTEGRATION_LOGIN_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function registerReminderSchedule(parent) {
    parent
        .command("schedule")
        ...
        if (opts.fireAt)
            body.fireAt = opts.fireAt;
        if (opts.in)
            body.delaySeconds = parseDuration(opts.in);
        if (opts.cadence)
            body.repeat = opts.cadence;
        const res = await client.request("POST", `/internal/agent/${encodeURIComponent(ctx.agentId)}/reminders`, body);
        if (!res.ok)
            fail("REMINDER_SCHEDULE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerReminderList(parent) {
    parent
        .command("list")
        ...
        const res = await client.request("GET", path);
        if (!res.ok)
            fail("REMINDER_LIST_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerReminderCancel(parent) {
    parent
        .command("cancel")
        ...
        if (!res.ok)
            fail("REMINDER_CANCEL_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write("Reminder cancelled\n");
    });
}
function registerReminderSnooze(parent) {
    parent
        .command("snooze")
        ...
        if (!res.ok)
            fail("REMINDER_SNOOZE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerReminderUpdate(parent) {
    parent
        .command("update")
        ...
        if (!res.ok)
            fail("REMINDER_UPDATE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
function registerReminderLog(parent) {
    parent
        .command("log")
        ...
        if (!res.ok)
            fail("REMINDER_LOG_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
// ---------------------------------------------------------------------------
// Action: prepare
// ---------------------------------------------------------------------------
function registerActionPrepare(parent) {
    parent
        .command("prepare")
        ...
        if (!res.ok)
            fail("ACTION_PREPARE_FAILED", res.error ?? `HTTP ${res.status}`);
        process.stdout.write(JSON.stringify(res.data, null, 2) + "\n");
    });
}
// ---------------------------------------------------------------------------
